...
function player(name,avatar,sessionId){ return { id:uid(), sessionId, name:clean(name), avatar:AV.includes(avatar)?avatar:AV[0], gender:null, connected:true, score:0, alive:true, rolePublic:null, isBot:false, geniusColorIndex:null, voiceRoom:'MAIN',voiceSpeaking:false }; }
function botPlayer(r){const n=r.players.filter(p=>p.isBot).length+1;return {id:uid(),sessionId:'bot-'+uid(),name:'소소봇'+n,avatar:AV[(n+2)%AV.length],gender:n%2?'male':'female',connected:true,score:0,alive:true,rolePublic:null,isBot:true,geniusColorIndex:null,voiceRoom:'MAIN',voiceSpeaking:false};}
...
io.on('connection', s => {
  s.on('geniusAuth',(d,cb)=>{try{const name=clean(d.name),pw=String(d.password||'');if(pw.length<4)throw Error('비밀번호는 4자 이상 입력해주세요.');let a=geniusAccounts[name];if(!a){a=geniusAccounts[name]={password:hashPassword(pw),garnets:1,wins:0,owned:[],equippedFrame:'',equippedBadge:''};saveAccounts()}else if(a.password!==hashPassword(pw))throw Error('비밀번호가 올바르지 않습니다.');a.owned=a.owned||[];a.equippedFrame=a.equippedFrame||'';a.equippedBadge=a.equippedBadge||'';s.data.geniusName=name;const rr=rooms.get(s.data.roomCode),rp=rr&&by(rr,s.data.playerId);if(rp){rp.geniusName=name;if(['genius','stockwar','deathmatch','indianpoker'].includes(rr.game)||['genius','deathmatch'].includes(rr.parentGame)){rp.name=name;rp.avatar='💎';}emitRoom(rr)}cb?.({ok:true,profile:{name,garnets:a.garnets||0,wins:a.wins||0,owned:a.owned||[],equippedFrame:a.equippedFrame||'',equippedBadge:a.equippedBadge||''}})}catch(e){cb?.({ok:false,error:e.message})}});
  ...
  s.on('voiceSpeaking',(d)=>{const r=rooms.get(s.data.roomCode),p=r&&by(r,s.data.playerId);if(!r||!p||!['genius','stockwar'].includes(r.game))return;p.voiceSpeaking=!!d.speaking;emitRoom(r)});
  ...
  s.on('voiceJoin',(d,cb)=>{try{const r=rooms.get(s.data.roomCode),p=by(r,s.data.playerId);if(!r||!p||!['genius','stockwar'].includes(r.game))throw Error('지니어스게임에서만 음성을 사용할 수 있습니다.');p.voiceRoom=p.voiceRoom||'MAIN';p.voiceSpeaking=false;emitRoom(r);cb?.({ok:true,room:p.voiceRoom});}catch(e){cb?.({ok:false,error:e.message})}});
