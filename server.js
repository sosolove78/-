...
function indianSetup(r){resetRound(r);const ps=active(r);if(ps.length!==2)throw Error('인디언 포커는 정확히 2명이 필요합니다.');if(ps.some(p=>!p.geniusName))throw Error('데스매치는 두 플레이어 모두 지니어스 프로필 로그인이 필요합니다.');const bet=Math.max(1,+r.settings.deathmatchGarnetBet||1);for(const p of ps){const a=geniusAccounts[p.geniusName];if(!a||a.garnets<bet)throw Error(`${p.name}님의 가넷이 부족합니다. 최소 ${bet}가넷이 필요합니다.`)}for(const p of ps)geniusAccounts[p.geniusName].garnets-=bet;saveAccounts();r.game='indianpoker';r.parentGame='deathmatch';r.round=1;r.indian={round:1,chips:Object.fromEntries(ps.map(p=>[p.id,20])),cards:{},bets:{},pot:0,currentBet:0,turnPlayerId:null,acted:[],lastAction:'룰 설명을 확인해주세요.',showdown:null,winnerId:null,briefingReady:[],garnetBet:bet,garnetPot:bet*2};r.phase='indianBriefing';setTimer(r,120,()=>indianStartRound(r));emitRoom(r)}
