...
io.on('connection', s => {
  s.on('siteStatus',(d,cb)=>cb?.({ok:true,maintenance:maintenanceMode,admin:adminOK(s,d)}));
  ...
  s.on('adminMaintenance',(d,cb)=>{if(!adminOK(s,d))return cb?.({ok:false,error:'관리자 인증이 필요합니다.'});maintenanceMode=!!d.enabled;io.emit('maintenanceChanged',{enabled:maintenanceMode});cb?.({ok:true,enabled:maintenanceMode})});
  ...
  s.on('createRoom',(d,cb)=>{try{if(maintenanceMode&&!s.data.adminToken)throw Error('SITE_MAINTENANCE');const game=LIMITS[d.game]?d.game:'liar';const sid=String(d.sessionId||uid()),p=player(d.name,d.avatar,sid),r=newRoom(game,d.mode,d.password,p);rooms.set(r.code,r);bind(s,r,p);cb?.({ok:true,code:r.code,playerId:p.id,sessionId:sid});emitRoom(r);}catch(e){cb?.({ok:false,error:e.message});}});
  s.on('joinRoom',(d,cb)=>{try{if(maintenanceMode&&!s.data.adminToken)throw Error('SITE_MAINTENANCE');const r=rooms.get(String(d.code||'').toUpperCase());if(!r)throw Error('방을 찾을 수 없습니다.');if(r.password&&r.password!==String(d.password||''))throw Error('비밀번호가 올바르지 않습니다.');const sid=String(d.sessionId||uid()),old=r.players.find(p=>p.sessionId===sid);if(old){bind(s,r,old);cb?.({ok:true,code:r.code,playerId:old.id,sessionId:sid});emitRoom(r);return;}const max=LIMITS[r.game][1];if(r.players.length>=max)throw Error(`현재 게임은 최대 ${max}명입니다.`);if(r.phase!=='lobby')throw Error('게임 진행 중에는 새로 참가할 수 없습니다.');const p=player(d.name,d.avatar,sid);r.players.push(p);bind(s,r,p);cb?.({ok:true,code:r.code,playerId:p.id,sessionId:sid});emitRoom(r);}catch(e){cb?.({ok:false,error:e.message});}});
  s.on('resumeSession',(d,cb)=>{if(maintenanceMode&&!s.data.adminToken)return cb?.({ok:false,error:'SITE_MAINTENANCE'});const r=rooms.get(String(d.code||'').toUpperCase()),p=r?.players.find(x=>x.sessionId===d.sessionId);if(!r||!p)return cb?.({ok:false});bind(s,r,p);cb?.({ok:true,code:r.code,playerId:p.id,sessionId:d.sessionId});emitRoom(r);});
  ...
  s.on('startGame',(_,cb)=>{try{const r=rooms.get(s.data.roomCode);if(!r||r.hostId!==s.data.playerId)throw Error('방장만 시작할 수 있습니다.');const [min,max]=LIMITS[r.game];const n=active(r).length;if(n<min||n>max)throw Error(`${GAME_NAMES[r.game]}은 ${min}~${max}명으로 플레이할 수 있습니다.`);if(r.game==='liar')startLiar(r);else if(r.game==='mafia')mafiaSetup(r);else if(r.game==='rummi')rummiSetup(r);else if(r.game==='wordchain')wordchainSetup(r);else if(r.game==='forgery')forgerySetup(r);else if(r.game==='crocodile')crocodileSetup(r);else if(r.game==='telepathy')telepathySetup(r);else if(r.game==='genius'){r.parentGame='genius';r.game=r.settings.geniusGame||'stockwar';if(r.game==='zombie')zombieSetup(r);else if(r.game==='smuggling')smugglingSetup(r);else stockSetup(r);}else if(r.game==='stockwar')stockSetup(r);else if(r.game==='smuggling')smugglingSetup(r);cb?.({ok:true});}catch(e){cb?.({ok:false,error:e.message});}});
  ...
  s.on('backToLobby',(_,cb)=>{const r=rooms.get(s.data.roomCode);if(r&&r.hostId===s.data.playerId){clearTimer(r);if(r.parentGame==='genius'){r.game='genius';r.parentGame=null;r.stock=null;r.zombie=null;r.smuggling=null;}else if(r.parentGame==='deathmatch'){r.game='deathmatch';r.parentGame=null;r.indian=null;}r.phase='lobby';r.players.forEach(p=>{p.alive=true;p.role=null;p.rolePublic=null;p.claimedRole=null});r.loverIds=[];r.mafiaChats={team:[],dead:[]};emitRoom(r);cb?.({ok:true});}});
  ...
  s.on('nextRound',(_,cb)=>{try{const r=rooms.get(s.data.roomCode);if(!r||r.hostId!==s.data.playerId)throw Error('방장만 진행할 수 있습니다.');if(r.game==='liar')startLiar(r);else if(r.game==='mafia')mafiaSetup(r);else if(r.game==='rummi')rummiSetup(r);else if(r.game==='wordchain')wordchainSetup(r);else if(r.game==='forgery')forgerySetup(r);else if(r.game==='crocodile')crocodileSetup(r);else if(r.game==='telepathy')telepathySetup(r);else if(r.game==='genius'){r.parentGame='genius';r.game=r.settings.geniusGame||'stockwar';if(r.game==='zombie')zombieSetup(r);else if(r.game==='smuggling')smugglingSetup(r);else stockSetup(r);}else if(r.game==='stockwar')stockSetup(r);cb?.({ok:true});}catch(e){cb?.({ok:false,error:e.message});}});
