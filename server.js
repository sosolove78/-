...
io.on('connection', s => {
  ...
  s.on('geniusShopBuy',(d,cb)=>{try{const r=rooms.get(s.data.roomCode),p=r&&by(r,s.data.playerId);if(!r||r.game!=='genius'||r.phase!=='lobby'||!p?.geniusName)throw Error('지니어스게임 대기실에서만 구매할 수 있습니다.');const catalog={frame_gold:{price:5},frame_neon:{price:8},frame_crystal:{price:12},badge_trader:{price:10},badge_genius:{price:20},collect_coin:{price:15}};const item=catalog[d.itemId];if(!item)throw Error('상품을 찾을 수 없습니다.');const a=geniusAccounts[p.geniusName];a.owned=a.owned||[];if(a.owned.includes(d.itemId))throw Error('이미 보유한 상품입니다.');if((a.garnets||0)<item.price)throw Error('가넷이 부족합니다.');a.garnets-=item.price;a.owned.push(d.itemId);saveAccounts();emitRoom(r);cb?.({ok:true})}catch(e){cb?.({ok:false,error:e.message})}});
  s.on('geniusShopEquip',(d,cb)=>{try{const r=rooms.get(s.data.roomCode),p=r&&by(r,s.data.playerId);if(!r||r.game!=='genius'||r.phase!=='lobby'||!p?.geniusName)throw Error('지니어스게임 대기실에서만 장착할 수 있습니다.');const a=geniusAccounts[p.geniusName];a.owned=a.owned||[];if(!a.owned.includes(d.itemId))throw Error('보유하지 않은 상품입니다.');if(String(d.itemId).startsWith('frame_'))a.equippedFrame=d.itemId;else if(d.itemId==='badge_trader')a.equippedBadge='시장 지배자';else if(d.itemId==='badge_genius')a.equippedBadge='GENIUS';else throw Error('장착할 수 없는 상품입니다.');saveAccounts();emitRoom(r);cb?.({ok:true})}catch(e){cb?.({ok:false,error:e.message})}});
