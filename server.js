...
function normalizeAccount(name,a){a=a||{};a.approved=a.approved!==false;a.garnets=Number(a.garnets||0);a.wins=Number(a.wins||0);a.games=Number(a.games||0);a.owned=a.owned||[];a.equippedFrame=a.equippedFrame||'';a.equippedBadge=a.equippedBadge||'';a.createdAt=a.createdAt||Date.now();return a}
Object.entries(geniusAccounts).forEach(([n,a])=>normalizeAccount(n,a));saveAccounts();
const profileOf=(name,a)=>({name,garnets:a.garnets||0,wins:a.wins||0,games:a.games||0,approved:!!a.approved,owned:a.owned||[],equippedFrame:a.equippedFrame||'',equippedBadge:a.equippedBadge||''});
const adminOK=(s,d)=>!!(s.data.adminToken&&d&&d.token===s.data.adminToken);

const rooms = new Map();
const AV = ['🐶','🐱','🐰','🐻','🐼','🐨','🦊','🐯','🦁','🐸','🐵','🐧','🐥','🦄','🐙','🦖','👻','🤖','👽','🥷','🧙','🧛','🧚','🧑‍🚀','🕵️','👑','😎','🤠','🥳','😈'];
const ADMIN_PASSWORD = '4890';
const LIMITS = { liar:[3,10], mafia:[5,12], truth:[2,10], rummi:[2,4], wordchain:[2,10], genius:[4,10], stockwar:[4,10], zombie:[6,10], forgery:[3,4], crocodile:[2,4], telepathy:[2,4], deathmatch:[2,2], indianpoker:[2,2] };
const GAME_NAMES = { liar:'라이어게임', mafia:'마피아게임', truth:'진실게임', rummi:'루미큐브', wordchain:'끝말잇기', genius:'지니어스게임', stockwar:'주식전쟁', zombie:'좀비게임', forgery:'위조범', crocodile:'악어이빨게임', telepathy:'텔레파시', deathmatch:'데스매치', indianpoker:'인디언 포커' };
...
function resetForGameSwitch(r,game){ clearTimer(r); r.game=game; r.mode='normal'; r.phase='lobby'; r.round=0; r.players.forEach(p=>{p.alive=true;p.role=null;p.rolePublic=null;p.score=0;}); r.settings=baseSettings(); r.prompt=null; r.liarIds=[]; r.rummi=null; r.mafiaLog=[]; r.truthQuestion=null; r.truthTurnId=null; r.wordchain=null; r.forgery=null; r.crocodile=null; r.telepathy=null; r.indian=null; r.mafiaChats={team:[],dead:[]}; r.loverIds=[]; r.soldierShieldUsed={}; r.reporterUsed={}; resetRound(r); }
...
function publicState(r){
  const base={ code:r.code, hostId:r.hostId, game:r.game, gameName:GAME_NAMES[r.game], mode:r.mode, phase:r.phase, round:r.round,
    players:(assignGeniusColors(r),r.players.map(p=>({id:p.id,name:p.name,avatar:p.avatar,gender:p.gender||null,connected:p.connected,score:p.score,alive:p.alive,rolePublic:p.rolePublic,claimedRole:p.claimedRole||null,isBot:!!p.isBot,geniusName:p.geniusName||null,geniusColorIndex:p.geniusColorIndex,geniusGarnets:p.geniusName?(geniusAccounts[p.geniusName]?.garnets||0):null,geniusWins:p.geniusName?(geniusAccounts[p.geniusName]?.wins||0):null,geniusOwned:p.geniusName?(geniusAccounts[p.geniusName]?.owned||[]):[],geniusFrame:p.geniusName?(geniusAccounts[p.geniusName]?.equippedFrame||''):'',geniusBadge:p.geniusName?(geniusAccounts[p.geniusName]?.equippedBadge||''):''}))),
...
function awardGeniusRanking(r,ranking){
  if(r.geniusRewardsApplied)return;
  const rows=(ranking||[]).filter(x=>by(r,x.id));
  if(!rows.length)return;
  const rewards=[3,1,1];
  rows.forEach(row=>{const p=by(r,row.id),a=p?.geniusName&&geniusAccounts[p.geniusName];if(a)a.games=(a.games||0)+1});
  rows.slice(0,3).forEach((row,i)=>{const p=by(r,row.id),a=p?.geniusName&&geniusAccounts[p.geniusName];if(!a)return;a.garnets=(a.garnets||0)+rewards[i];if(i===0)a.wins=(a.wins||0)+1;});
  const winner=by(r,rows[0].id),last=rows[rows.length-1];
  r.geniusGift={winnerId:winner?.id||rows[0].id,eligibleIds:rows.map(x=>x.id).filter(id=>id!==rows[0].id&&id!==last?.id&&!!by(r,id)?.geniusName),done:false,recipientId:null};
  r.geniusRewardsApplied=true;
}
...
io.on('connection', s => {
  s.on('geniusRegister',(d,cb)=>{try{const name=clean(d.name),pw=String(d.password||'');if(name.length<2)throw Error('닉네임은 2자 이상 입력해주세요.');if(pw.length<4)throw Error('비밀번호는 4자 이상 입력해주세요.');if(geniusAccounts[name])throw Error('이미 사용 중인 닉네임입니다.');geniusAccounts[name]=normalizeAccount(name,{password:hashPassword(pw),approved:false,garnets:1,wins:0,games:0,owned:[]});saveAccounts();cb?.({ok:true,pending:true,message:'회원가입 신청이 완료되었습니다. 관리자 승인 후 로그인할 수 있습니다.'})}catch(e){cb?.({ok:false,error:e.message})}});
  s.on('geniusAuth',(d,cb)=>{try{const name=clean(d.name),pw=String(d.password||''),a=geniusAccounts[name];if(!a)throw Error('등록되지 않은 닉네임입니다. 먼저 회원가입을 해주세요.');normalizeAccount(name,a);if(a.password!==hashPassword(pw))throw Error('비밀번호가 올바르지 않습니다.');if(!a.approved)throw Error('관리자 승인 대기 중인 계정입니다.');s.data.geniusName=name;const rr=rooms.get(s.data.roomCode),rp=rr&&by(rr,s.data.playerId);if(rp){rp.geniusName=name;if(['genius','stockwar','zombie','deathmatch','indianpoker'].includes(rr.game)||['genius','deathmatch'].includes(rr.parentGame)){rp.name=name;rp.avatar='💎';}emitRoom(rr)}cb?.({ok:true,profile:profileOf(name,a)})}catch(e){cb?.({ok:false,error:e.message})}});
  ...
  s.on('adminMembers',(d,cb)=>{if(!adminOK(s,d))return cb?.({ok:false,error:'관리자 인증이 필요합니다.'});const members=Object.entries(geniusAccounts).map(([name,a])=>{normalizeAccount(name,a);return profileOf(name,a)}).sort((a,b)=>(a.approved===b.approved?0:a.approved?1:-1)||a.name.localeCompare(b.name));cb?.({ok:true,members,maintenance:maintenanceMode})});
  s.on('adminMemberUpdate',(d,cb)=>{try{if(!adminOK(s,d))throw Error('관리자 인증이 필요합니다.');const name=clean(d.name),a=geniusAccounts[name];if(!a)throw Error('회원을 찾을 수 없습니다.');normalizeAccount(name,a);if(d.action==='approve')a.approved=true;else if(d.action==='wins')a.wins=Math.max(0,Math.min(9999,Math.floor(+d.value||0)));else if(d.action==='garnets')a.garnets=Math.max(0,Math.min(999999,Math.floor(+d.value||0)));else if(d.action==='delete')delete geniusAccounts[name];else throw Error('지원하지 않는 작업입니다.');saveAccounts();cb?.({ok:true})}catch(e){cb?.({ok:false,error:e.message})}});
  ...
  s.on('geniusShopBuyGlobal',(d,cb)=>{try{const name=s.data.geniusName,a=name&&geniusAccounts[name];if(!a)throw Error('두뇌게임 프로필 로그인이 필요합니다.');const catalog={frame_bronze:{price:3,wins:1},frame_silver:{price:6,wins:3},frame_gold:{price:10,wins:5},frame_platinum:{price:16,wins:8},frame_diamond:{price:25,wins:12},frame_genius:{price:40,wins:20},badge_rookie:{price:3,wins:1},badge_negotiator:{price:7,wins:3},badge_trader:{price:10,wins:5},badge_mindreader:{price:14,wins:7},badge_mastermind:{price:22,wins:10},badge_genius:{price:35,wins:15},collect_coin:{price:8,wins:0},collect_crown:{price:15,wins:3},collect_trophy:{price:25,wins:7}};const item=catalog[d.itemId];if(!item)throw Error('상품을 찾을 수 없습니다.');a.owned=a.owned||[];if(a.owned.includes(d.itemId))throw Error('이미 보유한 상품입니다.');if((a.wins||0)<item.wins)throw Error(`이 상품은 우승 ${item.wins}회 이상부터 구매할 수 있습니다.`);if((a.garnets||0)<item.price)throw Error('가넷이 부족합니다.');a.garnets-=item.price;a.owned.push(d.itemId);saveAccounts();cb?.({ok:true,profile:{name,garnets:a.garnets||0,wins:a.wins||0,owned:a.owned||[],equippedFrame:a.equippedFrame||'',equippedBadge:a.equippedBadge||''}})}catch(e){cb?.({ok:false,error:e.message})}});
  s.on('geniusShopEquipGlobal',(d,cb)=>{try{const name=s.data.geniusName,a=name&&geniusAccounts[name];if(!a)throw Error('두뇌게임 프로필 로그인이 필요합니다.');a.owned=a.owned||[];if(!a.owned.includes(d.itemId))throw Error('보유하지 않은 상품입니다.');if(String(d.itemId).startsWith('frame_'))a.equippedFrame=d.itemId;else if(String(d.itemId).startsWith('badge_')){const names={badge_rookie:'신예 전략가',badge_negotiator:'협상의 달인',badge_trader:'시장 지배자',badge_mindreader:'심리 설계자',badge_mastermind:'마스터마인드',badge_genius:'THE GENIUS'};a.equippedBadge=names[d.itemId]||'';}else throw Error('장착할 수 없는 상품입니다.');saveAccounts();cb?.({ok:true,profile:{name,garnets:a.garnets||0,wins:a.wins||0,owned:a.owned||[],equippedFrame:a.equippedFrame||'',equippedBadge:a.equippedBadge||''}})}catch(e){cb?.({ok:false,error:e.message})}});
  s.on('geniusShopBuy',(d,cb)=>{try{const r=rooms.get(s.data.roomCode),p=r&&by(r,s.data.playerId);if(!r||r.game!=='genius'||r.phase!=='lobby'||!p?.geniusName)throw Error('지니어스게임 대기실에서만 구매할 수 있습니다.');const catalog={frame_bronze:{price:3,wins:1},frame_silver:{price:6,wins:3},frame_gold:{price:10,wins:5},frame_platinum:{price:16,wins:8},frame_diamond:{price:25,wins:12},frame_genius:{price:40,wins:20},badge_rookie:{price:3,wins:1},badge_negotiator:{price:7,wins:3},badge_trader:{price:10,wins:5},badge_mindreader:{price:14,wins:7},badge_mastermind:{price:22,wins:10},badge_genius:{price:35,wins:15},collect_coin:{price:8,wins:0},collect_crown:{price:15,wins:3},collect_trophy:{price:25,wins:7}};const item=catalog[d.itemId];if(!item)throw Error('상품을 찾을 수 없습니다.');const a=geniusAccounts[p.geniusName];a.owned=a.owned||[];if(a.owned.includes(d.itemId))throw Error('이미 보유한 상품입니다.');if((a.wins||0)<item.wins)throw Error(`이 상품은 우승 ${item.wins}회 이상부터 구매할 수 있습니다.`);if((a.garnets||0)<item.price)throw Error('가넷이 부족합니다.');a.garnets-=item.price;a.owned.push(d.itemId);saveAccounts();emitRoom(r);cb?.({ok:true})}catch(e){cb?.({ok:false,error:e.message})}});
    s.on('geniusShopEquip',(d,cb)=>{try{const r=rooms.get(s.data.roomCode),p=r&&by(r,s.data.playerId);if(!r||r.game!=='genius'||r.phase!=='lobby'||!p?.geniusName)throw Error('지니어스게임 대기실에서만 장착할 수 있습니다.');const a=geniusAccounts[p.geniusName];a.owned=a.owned||[];if(!a.owned.includes(d.itemId))throw Error('보유하지 않은 상품입니다.');if(String(d.itemId).startsWith('frame_'))a.equippedFrame=d.itemId;else if(String(d.itemId).startsWith('badge_')){const names={badge_rookie:'신예 전략가',badge_negotiator:'협상의 달인',badge_trader:'시장 지배자',badge_mindreader:'심리 설계자',badge_mastermind:'마스터마인드',badge_genius:'THE GENIUS'};a.equippedBadge=names[d.itemId]||'';}else throw Error('장착할 수 없는 상품입니다.');saveAccounts();emitRoom(r);cb?.({ok:true})}catch(e){cb?.({ok:false,error:e.message})}});
  ...
  s.on('endGame',(_,cb)=>{try{const r=rooms.get(s.data.roomCode);if(!r||r.hostId!==s.data.playerId)throw Error('방장만 게임을 종료할 수 있습니다.');clearTimer(r);clearTimeout(r.botTimer);if(r.parentGame==='genius'||['stockwar','zombie'].includes(r.game)){r.game='genius';r.parentGame=null;r.stock=null;r.zombie=null;}else if(r.parentGame==='deathmatch'||r.game==='indianpoker'){r.game='deathmatch';r.parentGame=null;r.indian=null;}r.phase='lobby';r.round=0;r.geniusGift=null;r.geniusRewardsApplied=false;r.players.forEach(p=>{p.alive=true;p.role=null;p.rolePublic=null;p.claimedRole=null});emitRoom(r);cb?.({ok:true})}catch(e){cb?.({ok:false,error:e.message})}});
  ...
  s.on('backToLobby',(_,cb)=>{const r=rooms.get(s.data.roomCode);if(r&&r.hostId===s.data.playerId){clearTimer(r);if(r.parentGame==='genius'){r.game='genius';r.parentGame=null;r.stock=null;r.zombie=null;}else if(r.parentGame==='deathmatch'){r.game='deathmatch';r.parentGame=null;r.indian=null;}r.phase='lobby';r.players.forEach(p=>{p.alive=true;p.role=null;p.rolePublic=null;p.claimedRole=null});r.loverIds=[];r.mafiaChats={team:[],dead:[]};emitRoom(r);cb?.({ok:true});}});
  ...
  s.on('nextRound',(_,cb)=>{try{const r=rooms.get(s.data.roomCode);if(!r||r.hostId!==s.data.playerId)throw Error('방장만 진행할 수 있습니다.');if(r.game==='liar')startLiar(r);else if(r.game==='truth')startTruth(r);else if(r.game==='mafia')mafiaSetup(r);else if(r.game==='rummi')rummiSetup(r);else if(r.game==='wordchain')wordchainSetup(r);else if(r.game==='forgery')forgerySetup(r);else if(r.game==='crocodile')crocodileSetup(r);else if(r.game==='telepathy')telepathySetup(r);else if(r.game==='deathmatch'){r.parentGame='deathmatch';indianSetup(r);}else if(r.game==='genius'){if(active(r).some(p=>!p.isBot&&!p.geniusName))throw Error('모든 실제 플레이어가 두뇌게임 프로필 로그인을 완료해야 합니다.');r.parentGame='genius';r.game=r.settings.geniusGame||'stockwar';if(r.game==='zombie')zombieSetup(r);else stockSetup(r);}else if(r.game==='stockwar')stockSetup(r);else if(r.game==='indianpoker')indianSetup(r);cb?.({ok:true});}catch(e){cb?.({ok:false,error:e.message});}});
