...
const LIMITS = { liar:[3,10], mafia:[5,12], truth:[2,10], rummi:[2,4], wordchain:[2,10], genius:[4,10], stockwar:[4,10], zombie:[6,10], forgery:[3,4], crocodile:[2,4], deathmatch:[2,2], indianpoker:[2,2] };
const GAME_NAMES = { liar:'라이어게임', mafia:'마피아게임', truth:'진실게임', rummi:'루미큐브', wordchain:'끝말잇기', genius:'지니어스게임', stockwar:'주식전쟁', zombie:'좀비게임', forgery:'위조범', crocodile:'악어이빨게임', deathmatch:'데스매치', indianpoker:'인디언 포커' };
...
function resetForGameSwitch(r,game){ clearTimer(r); r.game=game; r.mode='normal'; r.phase='lobby'; r.round=0; r.players.forEach(p=>{p.alive=true;p.role=null;p.rolePublic=null;p.score=0;}); r.settings=baseSettings(); r.prompt=null; r.liarIds=[]; r.rummi=null; r.mafiaLog=[]; r.truthQuestion=null; r.truthTurnId=null; r.wordchain=null; r.forgery=null; r.crocodile=null; r.indian=null; r.mafiaChats={team:[],dead:[]}; r.loverIds=[]; r.soldierShieldUsed={}; r.reporterUsed={}; resetRound(r); }
...
io.on('connection', s => {
  ...
  s.on('nextRound',(_,cb)=>{try{const r=rooms.get(s.data.roomCode);if(!r||r.hostId!==s.data.playerId)throw Error('방장만 진행할 수 있습니다.');if(r.game==='liar')startLiar(r);else if(r.game==='truth')startTruth(r);else if(r.game==='mafia')mafiaSetup(r);else if(r.game==='rummi')rummiSetup(r);else if(r.game==='wordchain')wordchainSetup(r);else if(r.game==='forgery')forgerySetup(r);else if(r.game==='crocodile')crocodileSetup(r);else if(r.game==='deathmatch'){r.parentGame='deathmatch';indianSetup(r);}else if(r.game==='genius'){if(active(r).some(p=>!p.isBot&&!p.geniusName))throw Error('모든 실제 플레이어가 두뇌게임 프로필 로그인을 완료해야 합니다.');r.parentGame='genius';r.game=r.settings.geniusGame||'stockwar';if(r.game==='zombie')zombieSetup(r);else stockSetup(r);}else if(r.game==='stockwar')stockSetup(r);else if(r.game==='indianpoker')indianSetup(r);cb?.({ok:true});}catch(e){cb?.({ok:false,error:e.message});}});
