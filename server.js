...
const adminOK=(s,d)=>!!(s.data.adminToken&&d&&d.token===s.data.adminToken);

const rooms = new Map();
const AV = ['🐶','🐱','🐰','🐻','🐼','🐨','🦊','🐯','🦁','🐸','🐵','🐧','🐥','🦄','🐙','🦖','👻','🤖','👽','🥷','🧙','🧛','🧚','🧑‍🚀','🕵️','👑','😎','🤠','🥳','😈'];
const ADMIN_PASSWORD = '4890';
const LIMITS = { liar:[3,10], mafia:[5,12], rummi:[2,4], wordchain:[2,10], genius:[4,10], stockwar:[4,10], zombie:[6,10], smuggling:[6,10], forgery:[3,4], crocodile:[2,4], telepathy:[2,4] };
const GAME_NAMES = { liar:'라이어게임', mafia:'마피아게임', rummi:'루미큐브', wordchain:'끝말잇기', genius:'지니어스게임', stockwar:'주식전쟁', zombie:'좀비게임', smuggling:'밀수', forgery:'위조범', crocodile:'악어이빨게임', telepathy:'텔레파시' };
...
function baseSettings(){ return { liarCount:1, roundTime:90, voteTime:30, liarFinalGuess:true, liarRoleMode:'liar', liarMatch:'5', liarTalkTime:25, truthLevel:'normal', rummiTurnTime:60, wordchainMode:'normal', wordchainTurnTime:10, wordchainLives:3, wordchainOneShot:false, geniusGame:'stockwar', stockTradeTime:90, stockGarnetMatch:false, stockRankHidden:true }; }
function newRoom(game,mode,password,p){ return { usedLiarWords:[], usedLiarQuestions:[], lastActivity:Date.now(), code:code(), hostId:p.id, players:[p], game, mode:mode||'normal', phase:'lobby', round:0, password:String(password||'').slice(0,24), settings:baseSettings(), submitted:[], answers:{}, votes:{}, liarIds:[], settingsVersion:1 }; }
...
function resetForGameSwitch(r,game){ clearTimer(r); r.game=game; r.mode='normal'; r.phase='lobby'; r.round=0; r.players.forEach(p=>{p.alive=true;p.role=null;p.rolePublic=null;p.score=0;}); r.settings=baseSettings(); r.prompt=null; r.liarIds=[]; r.rummi=null; r.mafiaLog=[]; r.truthQuestion=null; r.truthTurnId=null; r.wordchain=null; r.forgery=null; r.crocodile=null; r.telepathy=null; r.smuggling=null; r.mafiaChats={team:[],dead:[]}; r.loverIds=[]; r.soldierShieldUsed={}; r.reporterUsed={}; resetRound(r); }
...
function publicState(r){
  const base={ code:r.code, hostId:r.hostId, game:r.game, gameName:GAME_NAMES[r.game], mode:r.mode, phase:r.phase, round:r.round,
    players:(assignGeniusColors(r),r.players.map(p=>({id:p.id,name:p.name,avatar:p.avatar,gender:p.gender||null,connected:p.connected,score:p.score,alive:p.alive,rolePublic:p.rolePublic,claimedRole:p.claimedRole||null,isBot:!!p.isBot,geniusColorIndex:p.geniusColorIndex}))),
    ...
    mafiaDealer:r.mafiaDealer||'', mafiaNightStep:r.phase==='mafiaNight'?'all':(r.mafiaNightStep||null), mafiaCandidateId:r.mafiaCandidateId||null, executeVotesSubmitted:r.executeVotesSubmitted||[], mafiaAwards:r.mafiaAwards||[], mafiaVoteReveal:['mafiaDefense','mafiaExecuteVote'].includes(r.phase)?Object.entries(r.votes||{}).map(([v,t])=>({voter:by(r,v)?.name||'?',target:t==='abstain'?'기권':(by(r,t)?.name||'?')})):[],
    terroristPickId:r.terroristPickId||null, discussionStarterId:r.discussionStarterId||null, discussionDirection:r.discussionDirection||null, liarTalkOrder:r.liarTalkOrder||[], liarTalkIndex:r.liarTalkIndex||0, liarTieCandidates:r.liarTieCandidates||[], liarVoteRound:r.liarVoteRound||1,
    promptMeta:r.game==='liar'&&r.prompt?{category:r.prompt.category,min:r.prompt.min,max:r.prompt.max}:null };
  if(r.game==='zombie'&&r.zombie) Object.assign(base,{zombie:{round:r.zombie.round,lastReport:r.zombie.lastReport||null,message:r.zombie.message||'',result:r.zombie.result||null,briefingReady:r.zombie.briefingReady||[]}});
  if(r.game==='forgery'&&r.forgery) Object.assign(base,{forgery:{round:r.forgery.round,totalRounds:r.forgery.totalRounds,phase:r.phase,submitted:r.forgery.submitted||[],votesSubmitted:r.forgery.votesSubmitted||[],drawings:(r.phase==='forgeryVote'||r.phase==='forgeryResult'||r.phase==='forgeryFinal')?r.forgery.drawings:{},roundResult:r.forgery.roundResult||null,scores:r.forgery.scores||{},finalRanking:r.forgery.finalRanking||[]}});
  if(r.game==='crocodile'&&r.crocodile) Object.assign(base,{crocodile:{round:r.crocodile.round,turnPlayerId:r.crocodile.turnOrder[r.crocodile.turnIndex],teeth:r.crocodile.teeth,opened:r.crocodile.opened||[],lives:r.crocodile.lives,lastAction:r.crocodile.lastAction||'',bittenId:r.crocodile.bittenId||null,winnerId:r.crocodile.winnerId||null}});
  if(r.game==='telepathy'&&r.telepathy) Object.assign(base,{telepathy:{round:r.telepathy.round,totalRounds:r.telepathy.totalRounds,prompt:r.telepathy.prompt,mode:r.telepathy.mode,submitted:Object.keys(r.telepathy.answers||{}),answers:(r.phase==='telepathyResult'||r.phase==='telepathyFinal')?r.telepathy.answers:{},roundPoints:r.telepathy.roundPoints||{},scores:r.telepathy.scores||{},finalRanking:r.telepathy.finalRanking||[]}});
  if(r.game==='smuggling'&&r.smuggling) Object.assign(base,{smuggling:{round:r.smuggling.round,totalRounds:7,officerId:r.smuggling.officerId,briefingReady:r.smuggling.briefingReady||[],prices:r.smuggling.prices||{},declarations:r.smuggling.declarations||{},submitted:r.smuggling.submitted||[],inspectionTargets:r.smuggling.inspectionTargets||[],inspectionLimit:r.smuggling.round===7?3:2,roundResults:r.smuggling.roundResults||[],cash:Object.fromEntries(active(r).map(p=>[p.id,r.smuggling.cash[p.id]||0])),wanted:Object.fromEntries(active(r).map(p=>[p.id,r.smuggling.wanted[p.id]||0])),messages:r.smuggling.publicMessage||'',finalRanking:r.smuggling.finalRanking||[]}});
  if(r.game==='stockwar'&&r.stock) Object.assign(base,{stock:{round:r.stock.round,companies:r.stock.companies,news:r.stock.news,marketEvent:r.stock.marketEvent||null,phase:r.stock.phase,portfolioPublic:false,dealer:r.stock.dealer,tradeEndsAt:r.stock.tradeEndsAt||null,ranking:r.stock.ranking||[],volume:r.stock.volume||{},marketPulse:Object.fromEntries(r.stock.companies.map(c=>{const f=r.stock.netFlow?.[c.id]||0;return [c.id,f>=5?'강한 매수세':f>=2?'매수 우위':f<=-5?'강한 매도세':f<=-2?'매도 우위':'보통']})),loanUsers:active(r).filter(p=>r.stock.accounts?.[p.id]?.loanTaken).map(p=>p.id),whaleAlerts:(r.stock.whaleAlerts||[]).slice(-5),publicAlerts:(r.stock.publicAlerts||[]).slice(-8),briefingReady:r.stock.briefingReady||[],briefingTotal:active(r).filter(x=>!x.isBot).length,auction:r.stock.auction?{active:r.stock.auction.active,endsAt:r.stock.auction.endsAt,highBid:r.stock.auction.highBid,highBidderName:r.stock.auction.highBidderName,round:r.stock.auction.round}:null,marketHistory:(r.stock.marketHistory||[]).slice(-8),afterMarket:r.stock.afterMarket||null,garnetPot:r.stock.garnetPot||0,liveRanking:[]}});
...
function emitRoom(r){
  ...
  for(const p of r.players){
    ...
    if(r.game==='liar' && r.prompt && r.phase!=='lobby'){
      const bad=r.liarIds.includes(p.id); const foolMode=r.liarRoleMode==='fool';
      s.emit('privateData',{game:'liar',isLiar:foolMode?false:bad,isFoolMode:foolMode,roleMode:r.liarRoleMode,
        word:r.mode==='normal' ? (bad?(foolMode?r.spyWord:null):r.prompt.word) : null,
    ...
    if(r.game==='zombie' && r.zombie && r.phase!=='lobby'){ const z=r.zombie.players[p.id]; if(z)s.emit('privateData',{game:'zombie',initialRole:z.originalZombie?'originalZombie':'human',originalZombie:!!z.originalZombie,points:z.points||0,cure:z.cure||0,touchesThisRound:[...z.touched],touchLimit:r.zombie.round===3?1:2,pendingIncoming:Object.entries(r.zombie.pending||{}).filter(([_,q])=>q.targetId===p.id).map(([id,q])=>({id,fromId:q.fromId,fromName:by(r,q.fromId)?.name||'플레이어'})),messages:r.zombie.messages?.[p.id]||[]}); }
    if(r.game==='forgery' && r.forgery && r.phase!=='lobby'){ s.emit('privateData',{game:'forgery',word:r.forgery.words?.[p.id]||'',isImposter:r.forgery.imposterId===p.id}); }
    if(r.game==='smuggling' && r.smuggling && r.phase!=='lobby'){ const x=r.smuggling; s.emit('privateData',{game:'smuggling',hand:x.hands?.[p.id]||[],bag:x.bags?.[p.id]||[],cash:x.cash?.[p.id]||0,wanted:x.wanted?.[p.id]||0,messages:x.messages?.[p.id]||[],special:x.specials?.[p.id]||null,specialUsed:!!x.specialUsed?.[p.id]}); }
...
// ---------- Liar ----------
const FOOL_PAIRS=[['콜라','사이다'],['축구','농구'],['고양이','강아지'],['라면','우동'],['햄버거','피자'],['초밥','회'],['커피','녹차'],['맥주','소주'],['수박','멜론'],['딸기','체리'],['사과','배'],['바나나','파인애플'],['기차','버스'],['택시','지하철'],['비행기','헬리콥터'],['학교','학원'],['병원','약국'],['영화관','공연장'],['바다','수영장'],['산','공원'],['의사','간호사'],['경찰','소방관'],['가수','배우'],['축구선수','야구선수'],['노트북','태블릿'],['휴대폰','컴퓨터'],['냉장고','에어컨'],['침대','소파'],['연필','볼펜'],['우산','우비'],['모자','선글라스'],['치킨','피자'],['떡볶이','순대'],['김밥','주먹밥'],['된장찌개','김치찌개'],['아이스크림','빙수'],['케이크','도넛']];
const FOOL_MAP=new Map(FOOL_PAIRS.flatMap(([a,b])=>[[a,b],[b,a]]));
function chooseFoolWord(prompt){const paired=FOOL_MAP.get(prompt.word);if(paired&&words.some(x=>x.word===paired))return paired;const pool=words.filter(x=>x.category===prompt.category&&x.word!==prompt.word);return pool.length?pick(pool).word:pick(words.filter(x=>x.word!==prompt.word)).word;}
function pickUnused(pool,used,key){let avail=pool.filter(x=>!used.includes(String(x[key])));if(!avail.length){used.length=0;avail=pool}const x=pick(avail);used.push(String(x[key]));if(used.length>500)used.shift();return x;}
function liarTalkStart(r){const ps=active(r);const starter=pick(ps),idx=ps.findIndex(x=>x.id===starter.id),dir=Math.random()<.5?1:-1;r.discussionStarterId=starter.id;r.discussionDirection=dir===1?'right':'left';r.liarTalkOrder=Array.from({length:ps.length},(_,i)=>ps[(idx+dir*i+ps.length*2)%ps.length].id);r.liarTalkIndex=0;r.phase='liarTalk';setTimer(r,r.settings.liarTalkTime||25,()=>liarTalkNext(r));emitRoom(r)}
function liarTalkNext(r){if(r.phase!=='liarTalk')return;r.liarTalkIndex++;if(r.liarTalkIndex>=r.liarTalkOrder.length){r.phase='play';setTimer(r,r.settings.roundTime||90,()=>openLiarVote(r));emitRoom(r);return}setTimer(r,r.settings.liarTalkTime||25,()=>liarTalkNext(r));emitRoom(r)}
function startLiar(r){
  if(active(r).length<3) throw Error('라이어게임은 최소 3명입니다.'); resetRound(r); r.round++;r.scoredRound=null;r.liarVoteRound=1;r.liarTieCandidates=[];
  r.liarRoleMode=r.mode==='question'?'liar':(r.settings.liarRoleMode==='random'?(Math.random()<.5?'liar':'fool'):r.settings.liarRoleMode);if(r.liarRoleMode==='spy')r.liarRoleMode='fool';
  const n=r.liarRoleMode==='fool'?1:clamp(r.settings.liarCount,1,Math.min(3,active(r).length-1)); r.liarIds=shuffle(active(r)).slice(0,n).map(p=>p.id);
  r.prompt=r.mode==='question'?pickUnused(questions,r.usedLiarQuestions,'question'):pickUnused(words,r.usedLiarWords,'word'); r.spyWord=r.liarRoleMode==='fool'?chooseFoolWord(r.prompt):null;
  if(r.mode==='normal')liarTalkStart(r);else{r.phase='play';if(r.settings.roundTime)setTimer(r,r.settings.roundTime,()=>revealAnswers(r));emitRoom(r)}
}
...
function openLiarVote(r,candidates=null){ clearTimer(r); r.phase='vote';r.votes={};r.votesSubmitted=[];r.liarTieCandidates=candidates||[];setTimer(r,r.settings.voteTime||30,()=>tallyLiar(r));emitRoom(r); }
function tallyLiar(r){
  if(r.phase!=='vote')return;clearTimer(r);const c={};Object.values(r.votes).forEach(x=>c[x]=(c[x]||0)+1);const m=Math.max(0,...Object.values(c));const top=Object.keys(c).filter(x=>c[x]===m&&m);
  if(top.length>1&&r.liarVoteRound===1){r.liarVoteRound=2;r.phase='liarDefense';r.liarTieCandidates=top;r.voteResult={counts:c,tie:true,tieIds:top,roleMode:r.liarRoleMode};setTimer(r,30,()=>openLiarVote(r,top));emitRoom(r);return}
  const target=top.length===1?top[0]:null,caught=!!target&&r.liarIds.includes(target);r.voteResult={counts:c,targetId:target,liarIds:r.liarIds,caught,tie:top.length!==1,roleMode:r.liarRoleMode,citizenWord:r.mode==='normal'?r.prompt.word:null,spyWord:r.liarRoleMode==='fool'?r.spyWord:null};
  if(caught&&r.mode==='normal'&&r.settings.liarFinalGuess&&r.liarRoleMode!=='fool'){r.phase='liarCaught';emitRoom(r);setTimeout(()=>{if(r.phase!=='liarCaught')return;r.phase='liarGuess';r.guessingLiarId=target;setTimer(r,25,()=>{r.guessResult={attempted:false,answer:r.prompt.word};scoreLiarResult(r,false);r.phase='result';emitRoom(r)});emitRoom(r)},1800)}else{scoreLiarResult(r,false);r.phase='result';emitRoom(r)}
}
function scoreLiarResult(r,guessCorrect=false){if(r.scoredRound===r.round)return;r.scoredRound=r.round;const caught=r.voteResult?.caught;for(const p of r.players){if(r.liarIds.includes(p.id))p.score+=caught?(guessCorrect?2:0):3;else if(caught)p.score+=1}}
// ---------- Truth ----------
...
const FORGERY_PAIRS=[['펭귄','오리'],['고양이','호랑이'],['피자','햄버거'],['학교','회사'],['달','태양'],['사과','토마토'],['비행기','새'],['선풍기','풍차'],['우산','버섯'],['기차','지하철'],['수박','축구공'],['로봇','우주인'],['상어','돌고래'],['성','호텔'],['컵','화분'],['자동차','버스'],['촛불','전구'],['왕관','모자'],['선물상자','택배상자'],['눈사람','곰']];
function forgerySetup(r){resetRound(r);r.round=1;r.forgery={round:1,totalRounds:3,scores:Object.fromEntries(active(r).map(p=>[p.id,0]))};forgeryRound(r)}
function forgeryRound(r){clearTimer(r);const f=r.forgery,ps=active(r),pair=pick(FORGERY_PAIRS),flip=Math.random()<.5?pair:[pair[1],pair[0]];f.imposterId=pick(ps).id;f.words=Object.fromEntries(ps.map(p=>[p.id,p.id===f.imposterId?flip[1]:flip[0]]));f.commonWord=flip[0];f.imposterWord=flip[1];f.drawings={};f.submitted=[];f.votes={};f.votesSubmitted=[];f.roundResult=null;r.phase='forgeryDraw';setTimer(r,45,()=>forgeryOpenVote(r));emitRoom(r)}
function forgerySubmit(r,pid,image){if(r.phase!=='forgeryDraw')throw Error('지금은 그림을 제출할 수 없습니다.');if(r.forgery.submitted.includes(pid))throw Error('이미 제출했습니다.');const x=String(image||'');if(!/^data:image\/png;base64,/.test(x)||x.length>900000)throw Error('그림 데이터가 올바르지 않습니다.');r.forgery.drawings[pid]=x;r.forgery.submitted.push(pid);if(active(r).every(p=>r.forgery.submitted.includes(p.id)))forgeryOpenVote(r);else emitRoom(r)}
function forgeryOpenVote(r){if(r.phase!=='forgeryDraw')return;clearTimer(r);for(const p of active(r))if(!r.forgery.drawings[p.id])r.forgery.drawings[p.id]='';r.phase='forgeryVote';setTimer(r,30,()=>forgeryResolve(r));emitRoom(r)}
function forgeryVote(r,pid,target){if(r.phase!=='forgeryVote')throw Error('지금은 투표 시간이 아닙니다.');if(pid===target)throw Error('자기 자신에게 투표할 수 없습니다.');if(r.forgery.votesSubmitted.includes(pid))throw Error('이미 투표했습니다.');if(!by(r,target))throw Error('플레이어를 찾을 수 없습니다.');r.forgery.votes[pid]=target;r.forgery.votesSubmitted.push(pid);if(active(r).every(p=>r.forgery.votesSubmitted.includes(p.id)))forgeryResolve(r);else emitRoom(r)}
function forgeryResolve(r){if(r.phase!=='forgeryVote')return;clearTimer(r);const f=r.forgery,counts={};Object.values(f.votes).forEach(id=>counts[id]=(counts[id]||0)+1);const max=Math.max(0,...Object.values(counts)),top=Object.keys(counts).filter(id=>counts[id]===max);const caught=top.length===1&&top[0]===f.imposterId;if(caught){for(const p of active(r))if(p.id!==f.imposterId&&f.votes[p.id]===f.imposterId)f.scores[p.id]+=1}else f.scores[f.imposterId]+=2;f.roundResult={imposterId:f.imposterId,caught,commonWord:f.commonWord,imposterWord:f.imposterWord,counts};r.phase='forgeryResult';if(f.round>=f.totalRounds){f.finalRanking=active(r).map(p=>({id:p.id,score:f.scores[p.id]||0})).sort((a,b)=>b.score-a.score);r.phase='forgeryFinal';return emitRoom(r)}setTimer(r,8,()=>{f.round++;r.round=f.round;forgeryRound(r)});emitRoom(r)}
function crocodileSetup(r){resetRound(r);const ps=shuffle(active(r));r.round=1;r.crocodile={round:1,turnOrder:ps.map(p=>p.id),turnIndex:0,teeth:12,opened:[],bad:1+Math.floor(Math.random()*12),lives:Object.fromEntries(ps.map(p=>[p.id,3])),lastAction:'악어가 입을 벌렸습니다. 이빨 하나를 선택하세요!'};r.phase='crocodilePlay';emitRoom(r)}
const TELEPATHY_PROMPTS=['비 오는 날 가장 생각나는 음식은?','여름 하면 가장 먼저 떠오르는 것은?','겨울 하면 가장 먼저 떠오르는 것은?','야식으로 가장 먼저 생각나는 음식은?','편의점에서 가장 먼저 집을 것 같은 것은?','학교 하면 떠오르는 것은?','여행 갈 때 꼭 챙겨야 하는 것은?','무인도에 하나만 가져간다면?','좀비에게 쫓길 때 들고 갈 물건은?','치킨과 가장 잘 어울리는 것은?','라면에 가장 잘 어울리는 것은?','영화관 하면 떠오르는 음식은?','빨간색 하면 떠오르는 것은?','파란색 하면 떠오르는 것은?','동물원 하면 떠오르는 동물은?','바다 하면 떠오르는 것은?','아침에 일어나 가장 먼저 하는 것은?','스트레스 받을 때 가장 하고 싶은 것은?','친구에게 받고 싶은 선물은?','카페에서 가장 자주 시키는 메뉴는?','한국 대표 음식 하면 떠오르는 것은?','공포영화 하면 떠오르는 것은?','생일 하면 떠오르는 것은?','놀이공원 하면 가장 먼저 떠오르는 것은?','만원이 생기면 가장 먼저 사고 싶은 것은?','배달음식 하면 가장 먼저 떠오르는 것은?','휴대폰에서 가장 많이 쓰는 기능은?','시험 전날 가장 많이 하는 행동은?','잠이 안 올 때 하는 것은?','눈 오는 날 가장 하고 싶은 것은?'];
function telepathySetup(r){resetRound(r);r.round=1;r.telepathy={round:1,totalRounds:7,scores:Object.fromEntries(active(r).map(p=>[p.id,0])),used:[]};telepathyRound(r)}
function telepathyRound(r){clearTimer(r);const t=r.telepathy;let pool=TELEPATHY_PROMPTS.filter(x=>!t.used.includes(x));if(!pool.length){t.used=[];pool=TELEPATHY_PROMPTS.slice()}t.prompt=pick(pool);t.used.push(t.prompt);t.mode=(t.round===3||t.round===6)?'reverse':'match';t.answers={};t.roundPoints={};r.phase='telepathyAnswer';setTimer(r,25,()=>telepathyResolve(r));emitRoom(r)}
function telepathySubmit(r,pid,answer){if(r.phase!=='telepathyAnswer')throw Error('지금은 답변 시간이 아닙니다.');if(r.telepathy.answers[pid])throw Error('이미 답변했습니다.');answer=String(answer||'').trim().replace(/\s+/g,' ');if(!answer||answer.length>20)throw Error('답변은 1~20자로 입력해주세요.');r.telepathy.answers[pid]=answer;if(active(r).every(p=>r.telepathy.answers[p.id]))telepathyResolve(r);else emitRoom(r)}
function telepathyResolve(r){if(r.phase!=='telepathyAnswer')return;clearTimer(r);const t=r.telepathy,ps=active(r),norm=x=>String(x||'').toLowerCase().replace(/\s+/g,'').replace(/[!?.~,]/g,''),groups={};for(const p of ps){const a=t.answers[p.id]||'(미입력)';const k=norm(a);(groups[k]??=[]).push(p.id)}for(const p of ps){const a=t.answers[p.id];let pts=0;if(a){const n=groups[norm(a)].length;if(t.mode==='match')pts=n>=2?n:0;else pts=n===1?2:0}t.roundPoints[p.id]=pts;t.scores[p.id]=(t.scores[p.id]||0)+pts}r.phase='telepathyResult';if(t.round>=t.totalRounds){t.finalRanking=ps.map(p=>({id:p.id,score:t.scores[p.id]||0})).sort((a,b)=>b.score-a.score);r.phase='telepathyFinal';return emitRoom(r)}setTimer(r,7,()=>{t.round++;r.round=t.round;telepathyRound(r)});emitRoom(r)}
function crocodileAlive(r){return r.crocodile.turnOrder.filter(id=>(r.crocodile.lives[id]||0)>0&&by(r,id)?.connected)}
function crocodilePress(r,pid,tooth){const c=r.crocodile;if(r.phase!=='crocodilePlay'||c.turnOrder[c.turnIndex]!==pid)throw Error('내 차례가 아닙니다.');tooth=+tooth;if(tooth<1||tooth>c.teeth||c.opened.includes(tooth))throw Error('선택할 수 없는 이빨입니다.');c.opened.push(tooth);if(tooth===c.bad){c.lives[pid]=Math.max(0,c.lives[pid]-1);c.bittenId=pid;c.lastAction=`🐊 앙! ${by(r,pid).name}님이 물렸습니다. 목숨 -1`;if(crocodileAlive(r).length<=1){c.winnerId=crocodileAlive(r)[0]||null;r.phase='crocodileResult';return emitRoom(r)}return setTimer(r,2,()=>{c.round++;r.round=c.round;c.opened=[];c.bad=1+Math.floor(Math.random()*c.teeth);c.bittenId=null;let alive=crocodileAlive(r);c.turnIndex=c.turnOrder.indexOf(alive[Math.floor(Math.random()*alive.length)]);c.lastAction='악어가 다시 입을 벌렸습니다!';emitRoom(r)}),emitRoom(r)}c.lastAction=`🦷 ${by(r,pid).name}님이 ${tooth}번 이빨을 눌렀습니다. 안전!`;do{c.turnIndex=(c.turnIndex+1)%c.turnOrder.length}while((c.lives[c.turnOrder[c.turnIndex]]||0)<=0);emitRoom(r)}
...
function zombieCounts(r){const vals=Object.values(r.zombie.players);return {humans:vals.filter(x=>x.status==='human').length,zombies:vals.filter(x=>x.status==='zombie').length}}
function zombieSetup(r){
  resetRound(r);r.geniusGift=null;r.geniusRewardsApplied=false;const ps=active(r);if(ps.length<6||ps.length>10)throw Error('좀비게임은 6~10명으로 플레이할 수 있습니다.');
  const zc=ps.length>=8?2:1,zombies=new Set(shuffle(ps).slice(0,zc).map(p=>p.id));r.round=1;r.zombie={round:1,players:{},pending:{},messages:{},allPairs:new Set(),lastReport:null,message:'룰 설명을 확인하십시오. 전원이 준비하면 게임을 시작합니다.',result:null,briefingReady:[]};
  for(const p of ps){r.zombie.players[p.id]={status:zombies.has(p.id)?'zombie':'human',originalZombie:zombies.has(p.id),points:0,cure:1,touched:new Set(),infectedAt:null};r.zombie.messages[p.id]=[]}
  r.phase='zombieBriefing';setTimer(r,180,()=>zombieRevealRoles(r));emitRoom(r)
}
function zombieRevealRoles(r){clearTimer(r);r.phase='zombieRole';r.zombie.message='비밀 역할을 확인하십시오. 감염 여부는 게임 중 공개되지 않습니다.';setTimer(r,12,()=>zombieStartRound(r,1));emitRoom(r)}
function zombieStartRound(r,n){clearTimer(r);r.round=n;r.zombie.round=n;r.zombie.pending={};for(const z of Object.values(r.zombie.players))z.touched=new Set();r.phase='zombiePlay';r.zombie.message=n===3?'최종 라운드입니다. 이번 라운드는 단 한 명과만 접촉할 수 있습니다.':'디스코드와 비밀대화로 상대를 판단하고, 믿을 사람과 접촉하십시오.';setTimer(r,n===3?420:360,()=>zombieEndRound(r));emitRoom(r)}
function zombieEndRound(r){clearTimer(r);for(const p of active(r)){const z=r.zombie.players[p.id];if(z?.status==='human'&&z.touched.size===0){z.status='zombie';z.infectedAt=Date.now()}}const c=zombieCounts(r);r.zombie.lastReport={round:r.zombie.round,...c};if(r.zombie.round>=3)return zombieFinish(r);r.phase='zombieReport';r.zombie.message=`${r.zombie.round}라운드 종료 · 인간 ${c.humans}명 / 좀비 ${c.zombies}명`;setTimer(r,10,()=>zombieStartRound(r,r.zombie.round+1));emitRoom(r)}
function zombieFinish(r){const c=zombieCounts(r);const rows=active(r).map(p=>{const z=r.zombie.players[p.id];return {id:p.id,name:p.name,status:z.status,originalZombie:z.originalZombie,points:z.points||0}});const humans=rows.filter(x=>x.status==='human').sort((a,b)=>b.points-a.points);const infected=rows.filter(x=>x.status==='zombie'&&!x.originalZombie).sort((a,b)=>b.points-a.points);const originals=rows.filter(x=>x.originalZombie).sort((a,b)=>b.points-a.points);const ranking=humans.length?[...humans,...infected,...originals]:[...originals,...infected];r.zombie.result={humans:c.humans,zombies:c.zombies,ranking};r.phase='zombieResult';r.zombie.message=humans.length?'생존 인간이 남았습니다. 생존 인간의 승점을 기준으로 최종 순위를 공개합니다.':'모든 인간이 감염되었습니다. 최초 좀비의 승리입니다.';awardGeniusRanking(r,ranking.map((x,i)=>({...x,value:Math.max(0,1000-i)})));emitRoom(r)}
function zombiePairKey(a,b){return [a,b].sort().join(':')}
function zombieTouchRequest(r,pid,targetId){if(r.phase!=='zombiePlay')throw Error('지금은 접촉할 수 없습니다.');if(pid===targetId)throw Error('자기 자신과 접촉할 수 없습니다.');const b=by(r,targetId),za=r.zombie.players[pid],zb=r.zombie.players[targetId];if(!b?.connected||!za||!zb)throw Error('접촉할 수 없는 플레이어입니다.');const limit=r.zombie.round===3?1:2;if(za.touched.size>=limit)throw Error(`이번 라운드 접촉 기회 ${limit}회를 모두 사용했습니다.`);if(zb.touched.size>=limit)throw Error('상대 플레이어는 이번 라운드 접촉 기회를 모두 사용했습니다.');if(r.zombie.allPairs.has(zombiePairKey(pid,targetId)))throw Error('한 번 접촉한 상대와는 게임 중 다시 접촉할 수 없습니다.');if(Object.values(r.zombie.pending||{}).some(q=>(q.fromId===pid&&q.targetId===targetId)||(q.fromId===targetId&&q.targetId===pid)))throw Error('이미 접촉 요청이 진행 중입니다.');const id=uid();r.zombie.pending[id]={fromId:pid,targetId,round:r.zombie.round};emitRoom(r);return id}
function zombieTouchRespond(r,pid,requestId,accept){const q=r.zombie.pending?.[requestId];if(!q||q.targetId!==pid)throw Error('유효한 접촉 요청이 아닙니다.');delete r.zombie.pending[requestId];if(!accept){emitRoom(r);return false}if(r.phase!=='zombiePlay'||q.round!==r.zombie.round)throw Error('접촉 요청 시간이 지났습니다.');const za=r.zombie.players[q.fromId],zb=r.zombie.players[q.targetId],limit=r.zombie.round===3?1:2,key=zombiePairKey(q.fromId,q.targetId);if(za.touched.size>=limit||zb.touched.size>=limit)throw Error('접촉 가능 횟수를 초과했습니다.');if(r.zombie.allPairs.has(key))throw Error('이미 게임 중 접촉한 상대입니다.');za.touched.add(q.targetId);zb.touched.add(q.fromId);r.zombie.allPairs.add(key);if(za.status==='human'&&zb.status==='human'){za.points++;zb.points++}else if(za.status!==zb.status){const human=za.status==='human'?za:zb;human.status='zombie';human.infectedAt=Date.now()}emitRoom(r);return true}
function zombieCure(r,pid){if(r.phase!=='zombiePlay')throw Error('라운드 진행 중에만 치료제를 사용할 수 있습니다.');const z=r.zombie.players[pid];if(!z)throw Error('플레이어 정보를 찾을 수 없습니다.');if(!z.cure)throw Error('치료제를 이미 사용했습니다.');z.cure--;if(!z.originalZombie&&z.status==='zombie'&&z.infectedAt&&Date.now()-z.infectedAt<=5*60*1000){z.status='human';z.infectedAt=null}emitRoom(r)}
function zombieWhisper(r,pid,targetId,text){const p=by(r,pid),t=by(r,targetId);if(!p||!t||p.id===t.id)throw Error('대상을 찾을 수 없습니다.');const msg=String(text||'').trim().slice(0,160);if(!msg)throw Error('메시지를 입력하세요.');r.zombie.messages[p.id].push({from:p.id,to:t.id,name:p.name,text:msg,own:true});r.zombie.messages[t.id].push({from:p.id,to:t.id,name:p.name,text:msg,own:false});r.zombie.messages[p.id]=r.zombie.messages[p.id].slice(-60);r.zombie.messages[t.id]=r.zombie.messages[t.id].slice(-60);emitRoom(r)}
...
function stockInfoCard(r,p,target,ev,index){
  const source=INFO_SOURCES[(index+p.name.length+r.stock.round)%INFO_SOURCES.length],dir=ev[1]>0?'상승':'하락',mag=Math.abs(ev[1]),others=shuffle(r.stock.companies.filter(c=>c.id!==target.id));
  const tier=index%6;let text,grade='조각정보',reliability=4;
  if(tier===0)text=`이번 라운드 핵심 사건은 ${target.sector} 업종에서 발생합니다.`;
  else if(tier===1)text=`핵심 사건의 방향은 ${dir}입니다. 정확한 기업은 다른 정보와 조합해야 합니다.`;
  else if(tier===2)text=`이번 핵심 종목은 ${target.name} 또는 ${others[0]?.name} 중 하나입니다.`;
  else if(tier===3)text=`${others[0]?.name}은 이번 라운드 핵심 사건의 직접 대상이 아닙니다.`;
  else if(tier===4)text=`핵심 종목의 변동폭은 대략 ${Math.max(8,mag-7)}~${mag+7}% 범위로 예상됩니다.`;
  else {const correct=Math.random()<.75,rumorDir=correct?dir:(dir==='상승'?'하락':'상승');text=`시장소문: ${target.name}이 ${rumorDir}할 것이라는 이야기가 돌고 있습니다.`;grade='시장소문';reliability=3;}
  return {id:uid(),round:r.stock.round,text,grade,source:source.name,reliability,companyId:target.id,truth:{direction:dir,pct:ev[1]}};
}
...
function stockRoundIntermission(r,nextRound){clearTimer(r);r.phase='stockIntermission';r.stock.phase='intermission';r.stock.dealer=`7초 후 ROUND ${nextRound}가 시작됩니다. 지난 시장 기록을 확인하고 다음 판단을 준비하세요.`;r.deadline=Date.now()+7000;setTimer(r,7,()=>{r.stock.round=nextRound;stockPrepareRound(r)});emitRoom(r)}
function stockSetup(r){
  ...
  r.stock={round:1,companies,accounts:{},info:{},messages:{},dealerOffers:{},contractLog:{},tradeLog:{},missions:{},volume:{},netFlow:{},whaleAlerts:[],publicAlerts:[],news:'시장이 곧 개장합니다.',dealer:'주식전쟁을 시작합니다. 모든 플레이어는 1,000코인으로 시작합니다.',phase:'briefing',tradeEndsAt:null,scenario:[],garnetPot:0,botActedRound:{},briefingReady:[],dealerBoughtRound:{},sharedInfoRound:{},marketPulse:{},midAlert:null,marketHistory:[]};
  ...
  ps.forEach((p,i)=>{r.stock.accounts[p.id]={cash:1000,holdings:{},traded:new Set(),loanTaken:false,debt:0,transferRound:1,transferTotal:0,transferTo:{}};r.stock.info[p.id]=[];r.stock.messages[p.id]=[];r.stock.dealerOffers[p.id]=[];r.stock.contractLog[p.id]=[];r.stock.tradeLog[p.id]=[];r.stock.missions[p.id]=null});
...
function makeDealerOffers(r,p,target,sc,i){
  const others=shuffle(r.stock.companies.filter(c=>c.id!==target.id)),dir=sc.eventPct>0?'상승':'하락',marketDir=sc.market?.pct>0?'상승':sc.market?.pct<0?'하락':'중립';
  const pool=[
    {price:45,grade:'시장 단서',stars:3,companyId:null,label:'시장 전체',text:`이번 라운드 시장 전체 흐름은 ${marketDir} 쪽에 가깝습니다.`},
    {price:60,grade:'업종 단서',stars:3,companyId:target.id,label:target.sector,text:`이번 핵심 사건은 ${target.sector} 업종에서 발생합니다.`},
    {price:75,grade:'후보 단서',stars:4,companyId:target.id,label:'핵심 후보',text:`이번 핵심 종목은 ${target.name} 또는 ${others[0]?.name} 중 하나입니다.`},
    {price:90,grade:'방향 단서',stars:4,companyId:target.id,label:'시장 방향',text:`핵심 사건의 방향은 ${dir}이며, 정확한 종목은 다른 정보와 조합해야 합니다.`},
    {price:70,grade:'제외 단서',stars:3,companyId:others[0]?.id,label:others[0]?.name||'타 종목',text:`${others[0]?.name||'이 종목'}은 이번 라운드 핵심 사건의 직접 대상이 아닙니다.`},
    {price:80,grade:'변동성 단서',stars:4,companyId:null,label:'시장 변동성',text:`이번 라운드는 격동장 ${sc.turbulent?'가능성이 매우 높습니다':'가능성이 낮습니다'}.`}
  ];return shuffle(pool).slice(0,3).map(x=>({...x,id:uid(),bought:false,round:r.stock.round}));
}
function stockPrepareRound(r){
  if(!r.stock||r.stock.round>8)return stockFinish(r);r.round=r.stock.round;const sc=r.stock.scenario[r.stock.round-1],target=r.stock.companies.find(c=>c.id===sc.targetId);r.stock.pending=sc;r.stock.volume=Object.fromEntries(r.stock.companies.map(c=>[c.id,0]));r.stock.netFlow=Object.fromEntries(r.stock.companies.map(c=>[c.id,0]));r.stock.whaleAlerts=[];r.stock.marketEvent=sc.market;r.stock.marketPulse={};
  const ps=active(r);ps.forEach((p,i)=>{const acct=r.stock.accounts[p.id]||(r.stock.accounts[p.id]={cash:1000,holdings:{},traded:new Set(),loanTaken:false,debt:0});acct.transferRound=r.stock.round;acct.transferTotal=0;acct.transferTo={};acct.roundStartHoldings={...acct.holdings};acct.roundNet={};r.stock.info[p.id]=r.stock.info[p.id]||[];r.stock.messages[p.id]=r.stock.messages[p.id]||[];r.stock.contractLog[p.id]=r.stock.contractLog[p.id]||[];r.stock.tradeLog[p.id]=r.stock.tradeLog[p.id]||[];const second=r.stock.companies[(i+r.stock.round)%r.stock.companies.length];r.stock.info[p.id].push(stockInfoCard(r,p,target,[sc.eventText,sc.eventPct],i+r.stock.round));if(second&&second.id!==target.id){const src=INFO_SOURCES[(i+r.stock.round+2)%INFO_SOURCES.length];r.stock.info[p.id].push({id:uid(),round:r.stock.round,text:`${second.name}은 이번 라운드 핵심 사건의 직접 대상이 아닙니다. 이 회사의 기본 변동성은 ${second.volatility}/5입니다.`,grade:'보조정보',source:src.name,reliability:src.stars,companyId:second.id,truth:{directTarget:false}})}r.stock.dealerOffers[p.id]=makeDealerOffers(r,p,target,sc,i)});
  r.stock.news=sc.market.pct?`시장 브리핑 · ${sc.market.text}`:`${target.sector} 업종을 둘러싼 시장의 관심이 커지고 있습니다.`;
  const infoSec=35;r.stock.phase='info';r.phase='stockInfoReview';r.stock.dealer=`ROUND ${r.stock.round}. 지금부터 ${infoSec}초 동안 새 정보와 시장 상황을 확인하세요.`;
  setTimer(r,infoSec,()=>stockNegotiationStart(r,target,sc));emitRoom(r)
}
function stockNegotiationStart(r,target,sc){
  if(!r.stock)return;clearTimer(r);r.stock.phase='negotiation';r.phase='stockNegotiation';const sec=r.stock.round<=2?150:r.stock.round<=5?135:r.stock.round<=7?120:150;r.stock.dealer=`정보 확인이 끝났습니다. 지금부터 ${Math.floor(sec/60)}분${sec%60?' '+sec%60+'초':''} 동안 디스코드 대화와 1:1 비밀대화를 활용해 정보를 조합하고 전략을 세우세요.`;
...
function stockAuctionStart(r,target,sc){
  forceVoiceMain(r);r.stock.phase='auction';r.phase='stockAuction';r.stock.auction={active:true,round:r.stock.round,endsAt:Date.now()+45000,highBid:0,highBidderId:null,highBidderName:'없음',bids:{},info:{id:uid(),round:r.stock.round,grade:'★★★★★ 특급정보',source:'시장 내부자',reliability:5,companyId:target.id,text:`이번 핵심 종목은 ${target.name}이며 방향은 ${sc.eventPct>0?'상승':'하락'}입니다. 변동폭은 시장 수급에 따라 크게 달라질 수 있습니다.`,truth:{direction:sc.eventPct>0?'상승':'하락',pct:sc.eventPct}}};
  r.stock.dealer=`특급정보 경매를 시작합니다. 45초 동안 진행됩니다. 보유 현금과 현재 최고가를 확인하고 판단하세요.`;setTimer(r,45,()=>stockAuctionClose(r));emitRoom(r)
}
...
function stockOpenTrade(r){r.stock.phase='trade';r.phase='stockTrade';const sec=75;r.stock.dealer=`ROUND ${r.stock.round} 시장이 열렸습니다. 1분 15초 동안 거래하세요. 반복 매수·매도로 거래량을 부풀려도 수급 영향은 늘어나지 않습니다.`;r.stock.tradeEndsAt=Date.now()+sec*1000;r.stock.midAlert=null;setTimeout(()=>{if(r.phase!=='stockTrade'||!r.stock)return;const sc=r.stock.pending,target=r.stock.companies.find(c=>c.id===sc.targetId);const clues=[`${target.sector} 업종에 기관 주문이 집중되고 있습니다.`,`${target.name}의 주문 불균형이 평소보다 커지고 있습니다.`,sc.turbulent?'시장 변동성 지표가 위험 구간에 진입했습니다.':'시장 변동성은 아직 통상 범위입니다.'];r.stock.midAlert=pick(clues);r.stock.publicAlerts.push({text:`시장 속보 · ${r.stock.midAlert}`,at:Date.now()});r.stock.publicAlerts=r.stock.publicAlerts.slice(-8);r.stock.dealer=`시장 중간 속보가 도착했습니다. 남은 시간 동안 기존 판단을 다시 검토하세요.`;emitRoom(r)},35000);setTimer(r,sec,()=>stockClose(r));emitRoom(r)}
function stockClose(r){
  if(!r.stock||r.phase!=='stockTrade')return;clearTimer(r);const sc=r.stock.pending;
  for(const c of r.stock.companies){const base=c.id===sc.targetId?Math.round(sc.eventPct*1.10):Math.round(((c.stability||3)-(c.volatility||3))*0.8),market=sc.market?.pct||0,flow=clamp(Math.round((r.stock.netFlow[c.id]||0)*1.4),-18,18),pct=clamp(base+market+flow,-55,70),old=c.price;c.price=Math.max(15,Math.round(old*(1+pct/100)));c.change=Math.round((c.price/old-1)*1000)/10;c.history.push(c.price)}
  const c=r.stock.companies.find(x=>x.id===sc.targetId);r.stock.news=`긴급속보 · ${c.name}: ${sc.eventText}`;r.stock.marketHistory.push({round:r.stock.round,event:`${c.name}: ${sc.eventText}`,rows:r.stock.companies.map(x=>({id:x.id,name:x.name,change:x.change,pulse:(r.stock.netFlow[x.id]||0)>=5?'강한 매수세':(r.stock.netFlow[x.id]||0)>=2?'매수 우위':(r.stock.netFlow[x.id]||0)<=-5?'강한 매도세':(r.stock.netFlow[x.id]||0)<=-2?'매도 우위':'보통'}))});r.stock.phase='result';r.phase='stockResult';r.stock.dealer=`장이 마감되었습니다. 사건, 등락률, 순수 매수·매도세를 확인하세요. 20초 뒤 다음 단계로 진행됩니다.`;setTimer(r,20,()=>{const next=r.stock.round+1;if(next>8)stockFinish(r);else stockRoundIntermission(r,next)});emitRoom(r)
}
...
function stockDealerAnswer(r,p,q){
  ...
  const rules=[
   ...
   [/정보상점|딜러정보|정보구매|정보사/,()=>`딜러 정보상점은 매 라운드 추론 단서 3개를 제시하며 1인당 라운드 1개만 살 수 있습니다. 딜러 단서는 진실이지만 정답 전체를 직접 알려주지는 않습니다.`],
   [/비밀정보|개인정보/,()=>`각 플레이어는 매 라운드 서로 다른 비밀정보를 받습니다. 받은 정보는 PRIVATE TERMINAL의 내 비밀정보에서 확인할 수 있습니다.`],
   [/특급|경매|입찰/,()=>`3라운드와 6라운드에는 45초 동안 특급정보 경매가 열립니다. 가장 높은 금액을 입찰한 플레이어가 정보를 얻습니다.`],
   [/밀담|비밀대화|채팅/,()=>r.phase==='stockTrade'?'시장 거래 1분 15초 동안은 비밀대화가 잠깁니다. 거래 전 전략·협상 시간에 밀담을 이용하세요.':'오른쪽 참가자의 대화 버튼을 누르면 해당 플레이어와 1:1 비밀대화를 할 수 있습니다.'],
   ...
   [/협상|전략/,()=>`전략·협상은 R1~2 2분30초, R3~5 2분15초, R6~7 2분, FINAL 2분30초입니다. 디스코드와 비밀대화, 송금, 정보 조합을 활용하세요.`],
   [/시장거래|마켓|marketopen/,()=>`시장 거래는 1분 15초입니다. 거래 중에는 밀담이 잠기고, 매수·매도 수급은 최종 주가에도 영향을 줄 수 있습니다.`]
  ];
...
function awardGeniusRanking(r,ranking){ r.geniusGift=null; r.geniusRewardsApplied=true; }

function stockFinish(r){
  clearTimer(r);r.phase='stockFinalCounting';r.stock.phase='finalCounting';r.stock.dealer='게임 결과를 집계중입니다.';emitRoom(r);setTimeout(()=>stockFinishReveal(r),5000)
}
function stockFinishReveal(r){
  if(!r.stock||r.phase!=='stockFinalCounting')return;for(const p of active(r)){const a=r.stock.accounts[p.id];if(a?.debt){const repay=a.debt;a.cash-=repay;a.debt=0;a.debtRepaid=repay;r.stock.contractLog[p.id].push({text:`최종 대출 상환 ${repay}C`})}}r.phase='stockFinal';r.stock.phase='final';r.stock.dealer='모든 장이 종료되었습니다. 대출 상환을 반영한 최종 자산과 시장의 진실을 공개합니다.';r.stock.ranking=active(r).map(p=>({id:p.id,name:p.name,value:stockValue(r,p.id)})).sort((a,b)=>b.value-a.value);
  const awards=[];const tradeAll=Object.entries(r.stock.tradeLog).flatMap(([pid,x])=>x.map(t=>({...t,pid,name:by(r,pid)?.name})));if(tradeAll.length){const biggest=[...tradeAll].sort((a,b)=>b.amount-a.amount)[0];awards.push(`🐋 가장 과감한 거래 · ${biggest.name} ${biggest.amount}C`)}const whisperKing=active(r).map(p=>({name:p.name,n:(r.stock.messages[p.id]||[]).filter(m=>m.own).length})).sort((a,b)=>b.n-a.n)[0];if(whisperKing?.n)awards.push(`🤫 협상의 중심 · ${whisperKing.name} · 비밀대화 ${whisperKing.n}회`);const infoKing=active(r).map(p=>({name:p.name,n:(r.stock.accounts[p.id]?.purchasedInfo||[]).length})).sort((a,b)=>b.n-a.n)[0];if(infoKing?.n)awards.push(`🔎 정보 수집가 · ${infoKing.name} · 딜러 정보 ${infoKing.n}개 구매`);const leader=r.stock.ranking?.[0];if(leader)awards.push(`👑 시장의 지배자 · ${leader.name} · 최종 순자산 ${leader.value}C`);const loanUser=active(r).find(p=>r.stock.accounts[p.id]?.debtRepaid);if(loanUser)awards.push(`🏦 승부수 · ${loanUser.name} · 신용대출을 활용해 끝까지 투자`);
  r.stock.afterMarket={awards,rounds:r.stock.scenario.map(sc=>({round:sc.round,company:r.stock.companies.find(c=>c.id===sc.targetId)?.name,event:sc.eventText,pct:sc.eventPct})),whispers:[]};
...
// ---------- Smuggling / 밀수 ----------
const SMUG_ITEMS={goods:{name:'생활품',icon:'📦',base:25,illegal:false},perfume:{name:'향수',icon:'🧴',base:45,illegal:false},jewel:{name:'보석',icon:'💎',base:80,illegal:false},contraband:{name:'밀수품',icon:'☠️',base:140,illegal:true}};
function smPrices(){const o={};for(const [k,v] of Object.entries(SMUG_ITEMS)){const swing=k==='contraband'?rand(-30,35):rand(-22,25);o[k]=Math.max(10,Math.round(v.base*(100+swing)/100/5)*5)}return o}
function smOfficer(r,round){const ps=active(r);return ps[(round-1)%ps.length]?.id}
function smDealHand(){const keys=['goods','goods','perfume','perfume','jewel','jewel','contraband','contraband','contraband'];return Array.from({length:5},()=>pick(keys))}
function smugglingSetup(r){resetRound(r);const ps=active(r);if(ps.length<6||ps.length>10)throw Error('밀수는 6~10명으로 플레이할 수 있습니다.');r.round=0;r.smuggling={round:0,briefingReady:[],cash:Object.fromEntries(ps.map(p=>[p.id,500])),wanted:Object.fromEntries(ps.map(p=>[p.id,0])),hands:{},bags:{},declarations:{},submitted:[],inspectionTargets:[],roundResults:[],messages:Object.fromEntries(ps.map(p=>[p.id,[]])),specials:Object.fromEntries(ps.map((p,i)=>[p.id,['정보상','위조업자','브로커','내부자'][i%4]])),specialUsed:{},stats:Object.fromEntries(ps.map(p=>[p.id,{smuggled:0,caught:0,honest:0,transfers:0}])),publicMessage:'룰 설명을 확인해주세요.'};r.phase='smugglingBriefing';setTimer(r,180,()=>smStartRound(r));emitRoom(r)}
function smStartRound(r){clearTimer(r);const x=r.smuggling;if(x.round>=7)return smFinish(r);x.round++;r.round=x.round;x.officerId=smOfficer(r,x.round);x.prices=smPrices();x.hands={};x.bags={};x.declarations={};x.submitted=[];x.inspectionTargets=[];x.roundResults=[];for(const p of active(r))if(p.id!==x.officerId)x.hands[p.id]=smDealHand();r.phase='smugglingPack';x.publicMessage=x.round===7?'FINAL ROUND · 마지막 국경 통과입니다.':'가방에 넣을 물품 3개를 선택하세요.';setTimer(r,x.round===7?55:45,()=>smAutoPack(r));emitRoom(r)}
function smAutoPack(r){const x=r.smuggling;for(const p of active(r))if(p.id!==x.officerId&&!x.submitted.includes(p.id)){x.bags[p.id]=(x.hands[p.id]||[]).slice(0,3);x.declarations[p.id]={item:'goods',count:3};x.submitted.push(p.id)}smNegotiation(r)}
function smCheckPacked(r){const x=r.smuggling,smug=active(r).filter(p=>p.id!==x.officerId);if(smug.every(p=>x.submitted.includes(p.id)))smNegotiation(r)}
function smNegotiation(r){clearTimer(r);r.phase='smugglingNegotiate';r.smuggling.publicMessage='신고서가 접수되었습니다. 세관원을 설득하고 거래하세요.';setTimer(r,120,()=>smInspection(r));emitRoom(r)}
function smInspection(r){clearTimer(r);r.phase='smugglingInspect';r.smuggling.publicMessage='세관원은 검사할 가방을 선택하세요.';setTimer(r,45,()=>smResolve(r));emitRoom(r)}
function smResolve(r){clearTimer(r);const x=r.smuggling,off=by(r,x.officerId),limit=x.round===7?3:2;x.inspectionTargets=(x.inspectionTargets||[]).slice(0,limit);x.roundResults=[];for(const p of active(r)){if(p.id===x.officerId)continue;const bag=x.bags[p.id]||[],d=x.declarations[p.id]||{item:'goods',count:3},ins=x.inspectionTargets.includes(p.id),truth=bag.length===d.count&&bag.every(k=>k===d.item);let income=0,penalty=0,comp=0;if(ins){if(truth){income=bag.reduce((n,k)=>n+(x.prices[k]||0),0);comp=40;x.cash[p.id]+=income+comp;x.cash[off.id]-=comp;x.stats[p.id].honest++;}else{penalty=60+20*(x.wanted[p.id]||0);x.cash[p.id]-=penalty;x.cash[off.id]+=40;x.stats[p.id].caught++;}}else{income=bag.reduce((n,k)=>n+(x.prices[k]||0),0);x.cash[p.id]+=income;const n=bag.filter(k=>k==='contraband').length;if(n){x.wanted[p.id]+=n;x.stats[p.id].smuggled+=n}}x.roundResults.push({id:p.id,name:p.name,inspected:ins,truth,bag:ins?bag:[],income,penalty,compensation:comp});}r.phase='smugglingResult';x.publicMessage=`ROUND ${x.round} 정산 완료`;setTimer(r,25,()=>smStartRound(r));emitRoom(r)}
function smFinish(r){clearTimer(r);const x=r.smuggling;x.finalRanking=active(r).map(p=>({id:p.id,name:p.name,value:x.cash[p.id]||0})).sort((a,b)=>b.value-a.value);r.phase='smugglingFinal';x.publicMessage='모든 국경이 폐쇄되었습니다. 최종 자산을 공개합니다.';emitRoom(r)}

io.on('connection', s => {
  s.on('siteStatus',(d,cb)=>cb?.({ok:true,admin:adminOK(s,d)}));
  ...
  s.on('createRoom',(d,cb)=>{try{const game=LIMITS[d.game]?d.game:'liar';const sid=String(d.sessionId||uid()),p=player(d.name,d.avatar,sid),r=newRoom(game,d.mode,d.password,p);rooms.set(r.code,r);bind(s,r,p);cb?.({ok:true,code:r.code,playerId:p.id,sessionId:sid});emitRoom(r);}catch(e){cb?.({ok:false,error:e.message});}});
  ...
  s.on('switchGame',(d,cb)=>{try{const r=rooms.get(s.data.roomCode);if(!r||r.hostId!==s.data.playerId||r.phase!=='lobby')throw Error('대기실에서 방장만 게임을 바꿀 수 있습니다.');const g=d.game;if(!LIMITS[g]||['stockwar','zombie','smuggling'].includes(g))throw Error('지원하지 않는 게임입니다.');if(r.players.length>LIMITS[g][1])throw Error(`${GAME_NAMES[g]}은 최대 ${LIMITS[g][1]}명입니다.`);resetForGameSwitch(r,g);emitRoom(r);cb?.({ok:true})}catch(e){cb?.({ok:false,error:e.message})}});
  ...
  s.on('updateSettings',(d,cb)=>{try{const r=rooms.get(s.data.roomCode);if(!r||r.hostId!==s.data.playerId||r.phase!=='lobby')throw Error('대기실에서 방장만 설정할 수 있습니다.');if(d.mode)r.mode=d.mode;if(d.liarCount)r.settings.liarCount=+d.liarCount;if(d.roundTime!==undefined)r.settings.roundTime=+d.roundTime;if(d.voteTime)r.settings.voteTime=+d.voteTime;if(d.truthLevel)r.settings.truthLevel=d.truthLevel;if(d.liarFinalGuess!==undefined)r.settings.liarFinalGuess=!!d.liarFinalGuess;if(d.liarRoleMode)r.settings.liarRoleMode=d.liarRoleMode==='spy'?'fool':d.liarRoleMode;if(d.liarMatch)r.settings.liarMatch=d.liarMatch;if(d.liarTalkTime)r.settings.liarTalkTime=clamp(+d.liarTalkTime,15,45);if(d.rummiTurnTime)r.settings.rummiTurnTime=+d.rummiTurnTime;if(d.wordchainMode)r.settings.wordchainMode=d.wordchainMode;if(d.wordchainTurnTime)r.settings.wordchainTurnTime=+d.wordchainTurnTime;if(d.wordchainLives)r.settings.wordchainLives=+d.wordchainLives;if(d.wordchainOneShot!==undefined)r.settings.wordchainOneShot=!!d.wordchainOneShot;if(d.geniusGame&&['stockwar','zombie','smuggling'].includes(d.geniusGame))r.settings.geniusGame=d.geniusGame;if(d.deathmatchGame)r.settings.deathmatchGame=d.deathmatchGame;if(d.deathmatchGarnetBet!==undefined)r.settings.deathmatchGarnetBet=Math.max(1,Math.min(5,+d.deathmatchGarnetBet||1));if(d.stockTradeTime)r.settings.stockTradeTime=+d.stockTradeTime;if(d.stockGarnetMatch!==undefined)r.settings.stockGarnetMatch=!!d.stockGarnetMatch;if(d.stockRankHidden!==undefined)r.settings.stockRankHidden=!!d.stockRankHidden;emitRoom(r);cb?.({ok:true});}catch(e){cb?.({ok:false,error:e.message});}});
  s.on('startGame',(_,cb)=>{try{const r=rooms.get(s.data.roomCode);if(!r||r.hostId!==s.data.playerId)throw Error('방장만 시작할 수 있습니다.');const [min,max]=LIMITS[r.game];const n=active(r).length;if(n<min||n>max)throw Error(`${GAME_NAMES[r.game]}은 ${min}~${max}명으로 플레이할 수 있습니다.`);if(r.game==='liar'){const max=r.settings.liarMatch==='1'?1:5;if(r.round>=max){r.round=0;r.players.forEach(p=>p.score=0)}startLiar(r);}else if(r.game==='mafia')mafiaSetup(r);else if(r.game==='rummi')rummiSetup(r);else if(r.game==='wordchain')wordchainSetup(r);else if(r.game==='forgery')forgerySetup(r);else if(r.game==='crocodile')crocodileSetup(r);else if(r.game==='telepathy')telepathySetup(r);else if(r.game==='genius'){r.parentGame='genius';r.game=r.settings.geniusGame||'stockwar';if(r.game==='zombie')zombieSetup(r);else if(r.game==='smuggling')smugglingSetup(r);else stockSetup(r);}else if(r.game==='stockwar')stockSetup(r);else if(r.game==='smuggling')smugglingSetup(r);cb?.({ok:true});}catch(e){cb?.({ok:false,error:e.message});}});
  s.on('liarSpeakerDone',(_,cb)=>{try{const r=rooms.get(s.data.roomCode);if(!r||r.game!=='liar'||r.phase!=='liarTalk')throw Error('지금은 발언 단계가 아닙니다.');if(r.liarTalkOrder[r.liarTalkIndex]!==s.data.playerId&&r.hostId!==s.data.playerId)throw Error('현재 발언자만 완료할 수 있습니다.');liarTalkNext(r);cb?.({ok:true})}catch(e){cb?.({ok:false,error:e.message})}});
  ...
  s.on('stockLoan',(d,cb)=>{try{const r=rooms.get(s.data.roomCode),p=by(r,s.data.playerId);if(!r||r.game!=='stockwar'||!p||!r.stock)throw Error('주식전쟁에서만 대출할 수 있습니다.');if(['lobby','stockBriefing','stockFinalCounting','stockFinal'].includes(r.phase))throw Error('현재는 대출을 받을 수 없습니다.');const a=r.stock.accounts[p.id];if(!a)throw Error('계좌 정보를 찾을 수 없습니다.');if(a.loanTaken)throw Error('대출은 게임당 1회만 가능합니다.');const amount=Math.floor(Number(d.amount));if(!Number.isFinite(amount)||amount<1||amount>500)throw Error('대출 금액은 1~500C로 입력해주세요.');a.cash+=amount;a.loanTaken=true;a.debt=Math.ceil(amount*1.2);r.stock.contractLog[p.id]=r.stock.contractLog[p.id]||[];r.stock.contractLog[p.id].push({text:`신용대출 ${amount}C · 최종 상환 ${a.debt}C`});r.stock.publicAlerts=r.stock.publicAlerts||[];r.stock.publicAlerts.push({text:`${p.name}님이 신용대출을 이용했습니다.`,at:Date.now()});r.stock.publicAlerts=r.stock.publicAlerts.slice(-8);emitRoom(r);cb?.({ok:true,amount,debt:a.debt});}catch(e){cb?.({ok:false,error:e.message})}});
  s.on('stockTrade',(d,cb)=>{try{const r=rooms.get(s.data.roomCode),p=by(r,s.data.playerId);if(!r||r.phase!=='stockTrade'||!p)throw Error('지금은 거래할 수 없습니다.');const a=r.stock.accounts[p.id],c=r.stock.companies.find(x=>x.id===d.companyId),q=Math.max(1,Math.min(99,+d.qty||1));if(!c)throw Error('종목을 찾을 수 없습니다.');const amount=c.price*q;if(d.side==='buy'){if(a.cash<amount)throw Error('코인이 부족합니다.');a.cash-=amount;a.holdings[c.id]=(a.holdings[c.id]||0)+q}else{if((a.holdings[c.id]||0)<q)throw Error('보유 수량이 부족합니다.');a.holdings[c.id]-=q;a.cash+=amount}const initial=a.roundStartHoldings?.[c.id]||0,current=a.holdings[c.id]||0,prevPlayerNet=a.roundNet?.[c.id]||0,newPlayerNet=current-initial;a.roundNet=a.roundNet||{};a.roundNet[c.id]=newPlayerNet;r.stock.netFlow[c.id]=(r.stock.netFlow[c.id]||0)-prevPlayerNet+newPlayerNet;a.traded.add(c.id);r.stock.volume[c.id]=active(r).reduce((sum,x)=>sum+Math.abs(r.stock.accounts[x.id]?.roundNet?.[c.id]||0),0);r.stock.tradeLog[p.id].push({round:r.stock.round,side:d.side,companyId:c.id,company:c.name,qty:q,price:c.price,amount});emitRoom(r);cb?.({ok:true})}catch(e){cb?.({ok:false,error:e.message})}});
  s.on('stockShareInfo',(d,cb)=>{try{const r=rooms.get(s.data.roomCode),p=by(r,s.data.playerId),t=by(r,d.targetId);if(!r||r.game!=='stockwar'||!p||!t||p.id===t.id)throw Error('대상을 찾을 수 없습니다.');if(r.phase==='stockTrade')throw Error('시장 거래 중에는 정보카드를 공유할 수 없습니다.');if(r.stock.sharedInfoRound[p.id]===r.stock.round)throw Error('인증 정보카드는 라운드당 1번만 공유할 수 있습니다.');const card=(r.stock.info[p.id]||[]).find(x=>x.id===d.infoId);if(!card)throw Error('정보카드를 찾을 수 없습니다.');const msg={from:p.id,to:t.id,name:p.name,text:`[인증 정보카드] ${card.text}`,card:{grade:card.grade,round:card.round,source:card.source,text:card.text},own:true};r.stock.messages[p.id].push(msg);r.stock.messages[t.id].push({...msg,own:false});r.stock.sharedInfoRound[p.id]=r.stock.round;r.stock.contractLog[p.id].push({text:`${t.name}에게 인증 정보카드 공유`});emitRoom(r);cb?.({ok:true})}catch(e){cb?.({ok:false,error:e.message})}});
  ...
  s.on('stockBuyDealerInfo',(d,cb)=>{try{const r=rooms.get(s.data.roomCode),p=by(r,s.data.playerId);if(!r||r.game!=='stockwar'||!p)throw Error('주식전쟁 방이 아닙니다.');if(!['stockInfoReview','stockNegotiation','stockAuction'].includes(r.phase))throw Error('정보 구매는 정보 확인·협상·경매 단계에서만 가능합니다.');if(r.stock.dealerBoughtRound[p.id]===r.stock.round)throw Error('딜러 정보는 라운드당 1개만 구매할 수 있습니다.');const offer=(r.stock.dealerOffers?.[p.id]||[]).find(x=>x.id===d.offerId);if(!offer||offer.bought)throw Error('구매할 수 없는 정보입니다.');const a=r.stock.accounts[p.id];if(a.cash<offer.price)throw Error('코인이 부족합니다.');a.cash-=offer.price;offer.bought=true;r.stock.dealerBoughtRound[p.id]=r.stock.round;a.purchasedInfo=a.purchasedInfo||[];a.purchasedInfo.push(offer.id);r.stock.info[p.id].push({id:uid(),round:r.stock.round,text:offer.text,grade:`딜러 ${offer.grade}`,source:'공식 딜러',reliability:offer.stars,companyId:offer.companyId,dealerBought:true});r.stock.contractLog[p.id].push({text:`딜러 ${offer.grade} 구매 · ${offer.price}코인`});emitRoom(r);cb?.({ok:true})}catch(e){cb?.({ok:false,error:e.message})}});
  s.on('zombieTouch',(d,cb)=>{try{const r=rooms.get(s.data.roomCode);if(!r||r.game!=='zombie')throw Error('좀비게임 방이 아닙니다.');zombieTouchRequest(r,s.data.playerId,d.targetId);cb?.({ok:true})}catch(e){cb?.({ok:false,error:e.message})}});
  s.on('zombieTouchRespond',(d,cb)=>{try{const r=rooms.get(s.data.roomCode);if(!r||r.game!=='zombie')throw Error('좀비게임 방이 아닙니다.');const done=zombieTouchRespond(r,s.data.playerId,d.requestId,!!d.accept);cb?.({ok:true,done})}catch(e){cb?.({ok:false,error:e.message})}});
  s.on('zombieCure',(_,cb)=>{try{const r=rooms.get(s.data.roomCode);if(!r||r.game!=='zombie')throw Error('좀비게임 방이 아닙니다.');zombieCure(r,s.data.playerId);cb?.({ok:true,message:'치료제를 사용했습니다. 효과 여부는 공개되지 않습니다.'})}catch(e){cb?.({ok:false,error:e.message})}});
  s.on('zombieWhisper',(d,cb)=>{try{const r=rooms.get(s.data.roomCode);if(!r||r.game!=='zombie')throw Error('좀비게임 방이 아닙니다.');zombieWhisper(r,s.data.playerId,d.targetId,d.text);cb?.({ok:true})}catch(e){cb?.({ok:false,error:e.message})}});
  s.on('zombieBriefingReady',(_,cb)=>{try{const r=rooms.get(s.data.roomCode),p=by(r,s.data.playerId);if(!r||r.phase!=='zombieBriefing'||!p)throw Error('지금은 룰 설명 단계가 아닙니다.');if(!r.zombie.briefingReady.includes(p.id))r.zombie.briefingReady.push(p.id);const ready=active(r).filter(x=>!x.isBot).every(x=>r.zombie.briefingReady.includes(x.id));if(ready)zombieRevealRoles(r);else emitRoom(r);cb?.({ok:true})}catch(e){cb?.({ok:false,error:e.message})}});
  s.on('endGame',(_,cb)=>{try{const r=rooms.get(s.data.roomCode);if(!r||r.hostId!==s.data.playerId)throw Error('방장만 게임을 종료할 수 있습니다.');clearTimer(r);clearTimeout(r.botTimer);if(r.parentGame==='genius'||['stockwar','zombie','smuggling'].includes(r.game)){r.game='genius';r.parentGame=null;r.stock=null;r.zombie=null;r.smuggling=null;}else if(r.parentGame==='deathmatch'||r.game==='indianpoker'){r.game='deathmatch';r.parentGame=null;r.indian=null;}r.phase='lobby';r.round=0;r.geniusGift=null;r.geniusRewardsApplied=false;r.players.forEach(p=>{p.alive=true;p.role=null;p.rolePublic=null;p.claimedRole=null});emitRoom(r);cb?.({ok:true})}catch(e){cb?.({ok:false,error:e.message})}});
  ...
  s.on('forgerySubmit',(d,cb)=>{try{const r=rooms.get(s.data.roomCode);if(!r||r.game!=='forgery')throw Error('위조범 방이 아닙니다.');forgerySubmit(r,s.data.playerId,d.image);cb?.({ok:true})}catch(e){cb?.({ok:false,error:e.message})}});
  s.on('forgeryVote',(d,cb)=>{try{const r=rooms.get(s.data.roomCode);if(!r||r.game!=='forgery')throw Error('위조범 방이 아닙니다.');forgeryVote(r,s.data.playerId,d.targetId);cb?.({ok:true})}catch(e){cb?.({ok:false,error:e.message})}});
  s.on('crocodilePress',(d,cb)=>{try{const r=rooms.get(s.data.roomCode);if(!r||r.game!=='crocodile')throw Error('악어이빨게임 방이 아닙니다.');crocodilePress(r,s.data.playerId,d.tooth);cb?.({ok:true})}catch(e){cb?.({ok:false,error:e.message})}});
  s.on('telepathySubmit',(d,cb)=>{try{const r=rooms.get(s.data.roomCode);if(!r||r.game!=='telepathy')throw Error('텔레파시 방이 아닙니다.');telepathySubmit(r,s.data.playerId,d.answer);cb?.({ok:true})}catch(e){cb?.({ok:false,error:e.message})}});
  // Word Chain
  ...
  s.on('backToLobby',(_,cb)=>{const r=rooms.get(s.data.roomCode);if(r&&r.hostId===s.data.playerId){clearTimer(r);if(r.parentGame==='genius'){r.game='genius';r.parentGame=null;r.stock=null;r.zombie=null;r.smuggling=null;}else if(r.parentGame==='deathmatch'){r.game='deathmatch';r.parentGame=null;r.indian=null;}r.phase='lobby';if(r.game==='liar'){r.round=0;r.players.forEach(p=>p.score=0)}r.players.forEach(p=>{p.alive=true;p.role=null;p.rolePublic=null;p.claimedRole=null});r.loverIds=[];r.mafiaChats={team:[],dead:[]};emitRoom(r);cb?.({ok:true});}});
  ...
  s.on('smugglingBriefingReady',(_,cb)=>{try{const r=rooms.get(s.data.roomCode);if(!r||r.game!=='smuggling'||r.phase!=='smugglingBriefing')throw Error('현재 룰 설명 단계가 아닙니다.');if(!r.smuggling.briefingReady.includes(s.data.playerId))r.smuggling.briefingReady.push(s.data.playerId);const humans=active(r).filter(p=>!p.isBot);if(humans.every(p=>r.smuggling.briefingReady.includes(p.id)))smStartRound(r);else emitRoom(r);cb?.({ok:true})}catch(e){cb?.({ok:false,error:e.message})}});
  s.on('smugglingPack',(d,cb)=>{try{const r=rooms.get(s.data.roomCode),x=r?.smuggling,p=by(r,s.data.playerId);if(!r||r.game!=='smuggling'||r.phase!=='smugglingPack'||!p||p.id===x.officerId)throw Error('지금은 가방을 제출할 수 없습니다.');const idx=[...new Set((d.indices||[]).map(Number))];if(idx.length!==3||idx.some(i=>i<0||i>=(x.hands[p.id]||[]).length))throw Error('물품 3개를 선택해주세요.');const item=String(d.declareItem||'');if(!SMUG_ITEMS[item])throw Error('신고 물품을 선택해주세요.');x.bags[p.id]=idx.map(i=>x.hands[p.id][i]);x.declarations[p.id]={item,count:3};if(!x.submitted.includes(p.id))x.submitted.push(p.id);smCheckPacked(r);emitRoom(r);cb?.({ok:true})}catch(e){cb?.({ok:false,error:e.message})}});
  s.on('smugglingInspect',(d,cb)=>{try{const r=rooms.get(s.data.roomCode),x=r?.smuggling;if(!r||r.game!=='smuggling'||r.phase!=='smugglingInspect'||s.data.playerId!==x.officerId)throw Error('세관원만 검사 대상을 정할 수 있습니다.');const ids=[...new Set(d.ids||[])].filter(id=>id!==x.officerId&&by(r,id));const lim=x.round===7?3:2;if(ids.length>lim)throw Error(`검사는 최대 ${lim}명입니다.`);x.inspectionTargets=ids;smResolve(r);cb?.({ok:true})}catch(e){cb?.({ok:false,error:e.message})}});
  s.on('smugglingMessage',(d,cb)=>{try{const r=rooms.get(s.data.roomCode),x=r?.smuggling,from=by(r,s.data.playerId),to=by(r,d.toId),text=clean(d.text).slice(0,160);if(!r||r.game!=='smuggling'||!['smugglingNegotiate','smugglingInspect'].includes(r.phase)||!from||!to||from.id===to.id||!text)throw Error('메시지를 보낼 수 없습니다.');const m={from:from.id,to:to.id,fromName:from.name,text,at:Date.now()};x.messages[from.id].push({...m,own:true});x.messages[to.id].push({...m,own:false});emitRoom(r);cb?.({ok:true})}catch(e){cb?.({ok:false,error:e.message})}});
  s.on('smugglingPay',(d,cb)=>{try{const r=rooms.get(s.data.roomCode),x=r?.smuggling,from=by(r,s.data.playerId),to=by(r,d.toId),amount=Math.max(1,Math.min(200,Math.floor(+d.amount||0)));if(!r||r.game!=='smuggling'||!['smugglingNegotiate','smugglingInspect'].includes(r.phase)||!from||!to||from.id===to.id)throw Error('송금할 수 없습니다.');if((x.cash[from.id]||0)<amount)throw Error('보유 C가 부족합니다.');x.cash[from.id]-=amount;x.cash[to.id]+=amount;x.stats[from.id].transfers+=amount;emitRoom(r);cb?.({ok:true})}catch(e){cb?.({ok:false,error:e.message})}});
  s.on('nextRound',(_,cb)=>{try{const r=rooms.get(s.data.roomCode);if(!r||r.hostId!==s.data.playerId)throw Error('방장만 진행할 수 있습니다.');if(r.game==='liar'){const max=r.settings.liarMatch==='1'?1:5;if(r.round>=max){r.round=0;r.players.forEach(p=>p.score=0)}startLiar(r);}else if(r.game==='mafia')mafiaSetup(r);else if(r.game==='rummi')rummiSetup(r);else if(r.game==='wordchain')wordchainSetup(r);else if(r.game==='forgery')forgerySetup(r);else if(r.game==='crocodile')crocodileSetup(r);else if(r.game==='telepathy')telepathySetup(r);else if(r.game==='genius'){r.parentGame='genius';r.game=r.settings.geniusGame||'stockwar';if(r.game==='zombie')zombieSetup(r);else if(r.game==='smuggling')smugglingSetup(r);else stockSetup(r);}else if(r.game==='stockwar')stockSetup(r);cb?.({ok:true});}catch(e){cb?.({ok:false,error:e.message});}});
