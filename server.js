...
const LIMITS = { liar:[3,10], mafia:[5,12], truth:[2,10], rummi:[2,4], wordchain:[2,10], stockwar:[4,10] };
const GAME_NAMES = { liar:'라이어게임', mafia:'마피아게임', truth:'진실게임', rummi:'루미큐브', wordchain:'끝말잇기', stockwar:'주식전쟁' };
...
function publicState(r){
  ...
  if(r.game==='stockwar'&&r.stock) Object.assign(base,{stock:{round:r.stock.round,companies:r.stock.companies,news:r.stock.news,phase:r.stock.phase,portfolioPublic:false,dealer:r.stock.dealer,tradeEndsAt:r.stock.tradeEndsAt||null,ranking:r.stock.ranking||[]}});
...
function emitRoom(r){
  ...
  for(const p of r.players){
    ...
    if(r.game==='stockwar' && r.stock && r.phase!=='lobby'){ const a=r.stock.accounts[p.id]||{cash:1000,holdings:{}}; s.emit('privateData',{game:'stockwar',cash:a.cash,holdings:a.holdings,info:r.stock.info[p.id]||[],messages:r.stock.messages[p.id]||[]}); }
...
const STOCK_COMPANIES=[
 {id:'serim',name:'세림전자',sector:'반도체·전자'},{id:'hanbit',name:'한빛바이오',sector:'제약·바이오'},
 {id:'mirae',name:'미래에너지',sector:'에너지'},{id:'star',name:'스타엔터',sector:'엔터테인먼트'},{id:'neo',name:'네오게임즈',sector:'게임'}];
const STOCK_EVENTS=[
 ['대규모 공급 계약 체결',18],['신제품 흥행 기대감',12],['해외 시장 진출 발표',15],['실적 전망 상향',10],
 ['핵심 프로젝트 성공',22],['규제 리스크 확대',-14],['예상 밖 실적 부진',-18],['경영진 논란',-12],['대형 계약 무산',-20],['시장 기대를 웃돈 성과',16]
];
...
function stockDealer(r,text){r.stock.dealer=text;r.mafiaLog=[];emitRoom(r)}
function stockSetup(r){resetRound(r);r.round=1;const ps=active(r);const companies=STOCK_COMPANIES.map(c=>({...c,price:100,change:0,history:[100]}));r.stock={round:1,companies,accounts:{},info:{},messages:{},news:'시장이 곧 개장합니다.',dealer:'주식전쟁을 시작합니다. 모든 플레이어는 1,000코인으로 시작합니다.',phase:'briefing',tradeEndsAt:null};for(const p of ps){r.stock.accounts[p.id]={cash:1000,holdings:{}};r.stock.info[p.id]=[];r.stock.messages[p.id]=[]}r.phase='stockBriefing';setTimer(r,8,()=>stockRound(r));emitRoom(r)}
function stockRound(r){if(!r.stock||r.stock.round>8)return stockFinish(r);r.round=r.stock.round;const target=pick(r.stock.companies),ev=pick(STOCK_EVENTS);r.stock.pending={id:target.id,text:ev[0],pct:ev[1]};r.stock.news=`${target.sector} 업종을 둘러싼 시장의 관심이 커지고 있습니다.`;for(const p of active(r)){const exact=Math.random()<.38;r.stock.info[p.id].push({round:r.stock.round,text:exact?`${target.name}은 이번 장 마감 후 ${ev[1]>0?'상승':'하락'}합니다.`:`이번 라운드에는 ${target.sector} 업종에서 큰 변동이 예상됩니다.`,grade:exact?'고급정보':'일반정보'})}r.stock.phase='trade';r.phase='stockTrade';r.stock.dealer=`ROUND ${r.stock.round}. 새로운 비밀정보가 도착했습니다. 지금부터 90초 동안 투자와 협상을 진행합니다.`;r.stock.tradeEndsAt=Date.now()+90000;setTimer(r,90,()=>stockClose(r));emitRoom(r)}
function stockClose(r){if(!r.stock||r.phase!=='stockTrade')return;clearTimer(r);const e=r.stock.pending;for(const c of r.stock.companies){let pct=c.id===e.id?e.pct:Math.round((Math.random()*10-5));const old=c.price;c.price=Math.max(20,Math.round(old*(1+pct/100)));c.change=Math.round((c.price/old-1)*1000)/10;c.history.push(c.price)}const c=r.stock.companies.find(x=>x.id===e.id);r.stock.news=`긴급속보 · ${c.name}: ${e.text}`;r.stock.phase='result';r.phase='stockResult';r.stock.dealer=`장이 마감되었습니다. ${c.name} 관련 속보입니다. 주가 변동을 확인하세요.`;setTimer(r,10,()=>{r.stock.round++;stockRound(r)});emitRoom(r)}
function stockFinish(r){clearTimer(r);r.phase='stockFinal';r.stock.phase='final';r.stock.dealer='모든 장이 종료되었습니다. 최종 자산을 공개합니다.';r.stock.ranking=active(r).map(p=>({id:p.id,name:p.name,value:stockValue(r,p.id)})).sort((a,b)=>b.value-a.value);const win=r.stock.ranking[0];if(win){const p=by(r,win.id);if(p?.geniusName&&geniusAccounts[p.geniusName]){geniusAccounts[p.geniusName].garnets=(geniusAccounts[p.geniusName].garnets||0)+5;geniusAccounts[p.geniusName].wins=(geniusAccounts[p.geniusName].wins||0)+1;saveAccounts()}}emitRoom(r)}

io.on('connection', s => {
  s.on('geniusAuth',(d,cb)=>{try{const name=clean(d.name),pw=String(d.password||'');if(pw.length<4)throw Error('비밀번호는 4자 이상 입력해주세요.');let a=geniusAccounts[name];if(!a){a=geniusAccounts[name]={password:hashPassword(pw),garnets:0,wins:0};saveAccounts()}else if(a.password!==hashPassword(pw))throw Error('비밀번호가 올바르지 않습니다.');s.data.geniusName=name;cb?.({ok:true,profile:{name,garnets:a.garnets||0,wins:a.wins||0}})}catch(e){cb?.({ok:false,error:e.message})}});
  ...
  s.on('createRoom',(d,cb)=>{try{const game=LIMITS[d.game]?d.game:'liar',sid=String(d.sessionId||uid()),p=player(d.name,d.avatar,sid),r=newRoom(game,d.mode,d.password,p);if(game==='stockwar'){if(!s.data.geniusName)throw Error('지니어스게임 로그인이 필요합니다.');p.geniusName=s.data.geniusName;}rooms.set(r.code,r);bind(s,r,p);cb?.({ok:true,code:r.code,playerId:p.id,sessionId:sid});emitRoom(r);}catch(e){cb?.({ok:false,error:e.message});}});
  s.on('joinRoom',(d,cb)=>{try{const r=rooms.get(String(d.code||'').toUpperCase());if(!r)throw Error('방을 찾을 수 없습니다.');if(r.password&&r.password!==String(d.password||''))throw Error('비밀번호가 올바르지 않습니다.');const sid=String(d.sessionId||uid()),old=r.players.find(p=>p.sessionId===sid);if(old){bind(s,r,old);cb?.({ok:true,code:r.code,playerId:old.id,sessionId:sid});emitRoom(r);return;}const max=LIMITS[r.game][1];if(r.players.length>=max)throw Error(`현재 게임은 최대 ${max}명입니다.`);if(r.phase!=='lobby')throw Error('게임 진행 중에는 새로 참가할 수 없습니다.');const p=player(d.name,d.avatar,sid);if(r.game==='stockwar'){if(!s.data.geniusName)throw Error('지니어스게임 로그인이 필요합니다.');p.geniusName=s.data.geniusName;}r.players.push(p);bind(s,r,p);cb?.({ok:true,code:r.code,playerId:p.id,sessionId:sid});emitRoom(r);}catch(e){cb?.({ok:false,error:e.message});}});
  ...
  s.on('stockTrade',(d,cb)=>{try{const r=rooms.get(s.data.roomCode),p=by(r,s.data.playerId);if(!r||r.phase!=='stockTrade'||!p)throw Error('지금은 거래할 수 없습니다.');const a=r.stock.accounts[p.id],c=r.stock.companies.find(x=>x.id===d.companyId),q=Math.max(1,Math.min(99,+d.qty||1));if(!c)throw Error('종목을 찾을 수 없습니다.');if(d.side==='buy'){const cost=c.price*q;if(a.cash<cost)throw Error('코인이 부족합니다.');a.cash-=cost;a.holdings[c.id]=(a.holdings[c.id]||0)+q}else{if((a.holdings[c.id]||0)<q)throw Error('보유 수량이 부족합니다.');a.holdings[c.id]-=q;a.cash+=c.price*q}emitRoom(r);cb?.({ok:true})}catch(e){cb?.({ok:false,error:e.message})}});
  ...
  s.on('nextRound',(_,cb)=>{try{const r=rooms.get(s.data.roomCode);if(!r||r.hostId!==s.data.playerId)throw Error('방장만 진행할 수 있습니다.');if(r.game==='liar')startLiar(r);else if(r.game==='truth')startTruth(r);else if(r.game==='mafia')mafiaSetup(r);else if(r.game==='rummi')rummiSetup(r);else if(r.game==='wordchain')wordchainSetup(r);else if(r.game==='stockwar')stockSetup(r);cb?.({ok:true});}catch(e){cb?.({ok:false,error:e.message});}});
