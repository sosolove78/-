...
function stockPrepareRound(r){
  ...
  r.stock.phase='info';r.phase='stockInfoReview';r.stock.dealer=`ROUND ${r.stock.round}. 지금부터 3분 동안 새 정보와 시장 상황을 확인하세요.`;
  setTimer(r,180,()=>stockNegotiationStart(r,target,sc));emitRoom(r)
}
function stockNegotiationStart(r,target,sc){
  if(!r.stock)return;clearTimer(r);r.stock.phase='negotiation';r.phase='stockNegotiation';r.stock.dealer=`정보 확인이 끝났습니다. 지금부터 5분 동안 밀담과 협상을 진행하고 투자 전략을 세우세요.`;
  setTimer(r,300,()=>{if([3,6].includes(r.stock.round))stockAuctionStart(r,target,sc);else stockOpenTrade(r)});emitRoom(r)
}
function stockAuctionStart(r,target,sc){
  r.stock.phase='auction';r.phase='stockAuction';r.stock.auction={active:true,round:r.stock.round,endsAt:Date.now()+120000,highBid:0,highBidderId:null,highBidderName:'없음',bids:{},info:{id:uid(),round:r.stock.round,grade:'★★★★★ 특급정보',source:'시장 내부자',reliability:5,companyId:target.id,text:`${target.name}은 이번 장 마감 후 ${sc.eventPct>0?'상승':'하락'}하며, 예상 변동폭은 약 ${Math.abs(sc.eventPct)}%입니다.`,truth:{direction:sc.eventPct>0?'상승':'하락',pct:sc.eventPct}}};
  r.stock.dealer=`특급정보 경매를 시작합니다. 2분 동안 입찰과 밀담을 진행할 수 있습니다.`;setTimer(r,120,()=>stockAuctionClose(r));emitRoom(r)
}
...
function stockClose(r){
  if(!r.stock||r.phase!=='stockTrade')return;clearTimer(r);const sc=r.stock.pending;for(const c of r.stock.companies){const base=c.id===sc.targetId?sc.eventPct:Math.round(Math.random()*7-3),market=sc.market?.pct||0,flow=clamp(Math.round((r.stock.netFlow[c.id]||0)/4),-8,8),pct=clamp(base+market+flow,-55,65),old=c.price;c.price=Math.max(20,Math.round(old*(1+pct/100)));c.change=Math.round((c.price/old-1)*1000)/10;c.history.push(c.price)}const c=r.stock.companies.find(x=>x.id===sc.targetId);r.stock.news=`긴급속보 · ${c.name}: ${sc.eventText}`;r.stock.phase='result';r.phase='stockResult';r.stock.dealer=`장이 마감되었습니다. ${c.name} 관련 속보입니다. 주가 변동과 거래량을 충분히 확인하세요.`;setTimer(r,120,()=>{const next=r.stock.round+1;if(next>8)stockFinish(r);else stockRoundIntermission(r,next)});emitRoom(r)
}
...
function stockDealerAnswer(r,p,q){
  ...
  if(/가넷/.test(text))return '가넷은 두뇌게임 프로필에 누적되는 별도 재화입니다. 주식전쟁의 코인과는 서로 다른 자산입니다.';
