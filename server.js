...
function makeTruth(r){ const level=r.settings.truthLevel||'normal';const targeted=active(r).length>2&&Math.random()<.42;const pool=targeted?truth.targeted[level]:truth[level];let q=pick(pool);if(targeted){const targets=active(r).filter(x=>x.id!==r.truthTurnId);if(targets.length)q=q.replaceAll('{target}',pick(targets).name);}r.truthQuestion=q; }
...
io.on('connection', s => {
  ...
  s.on('startGame',(_,cb)=>{try{const r=rooms.get(s.data.roomCode);if(!r||r.hostId!==s.data.playerId)throw Error('방장만 시작할 수 있습니다.');const [min,max]=LIMITS[r.game];const n=active(r).length;if(n<min||n>max)throw Error(`${GAME_NAMES[r.game]}은 ${min}~${max}명으로 플레이할 수 있습니다.`);if(r.game==='liar')startLiar(r);else if(r.game==='mafia')mafiaSetup(r);else if(r.game==='truth')startTruth(r);else if(r.game==='draw')drawSetup(r);else if(r.game==='rummi')rummiSetup(r);cb?.({ok:true});}catch(e){cb?.({ok:false,error:e.message});}});
