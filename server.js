...
const LIMITS = { liar:[3,10], mafia:[5,12], truth:[2,10], rummi:[2,4], wordchain:[2,10], genius:[4,10], stockwar:[4,10], zombie:[6,10], deathmatch:[2,2], indianpoker:[2,2] };
const GAME_NAMES = { liar:'라이어게임', mafia:'마피아게임', truth:'진실게임', rummi:'루미큐브', wordchain:'끝말잇기', genius:'지니어스게임', stockwar:'주식전쟁', zombie:'좀비게임', deathmatch:'데스매치', indianpoker:'인디언 포커' };
...
function publicState(r){
  ...
  if(r.game==='zombie'&&r.zombie) Object.assign(base,{zombie:{round:r.zombie.round,lastReport:r.zombie.lastReport||null,message:r.zombie.message||'',result:r.zombie.result||null}});
...
function emitRoom(r){
  ...
  for(const p of r.players){
    ...
    if(r.game==='zombie' && r.zombie && r.phase!=='lobby'){ const z=r.zombie.players[p.id]; if(z)s.emit('privateData',{game:'zombie',status:z.status,originalZombie:!!z.originalZombie,points:z.points||0,cure:z.cure||0,touchesThisRound:[...z.touched],infectedAt:z.infectedAt||null,cureUntil:z.infectedAt?z.infectedAt+10*60*1000:null,pendingIncoming:Object.entries(r.zombie.pending||{}).filter(([_,q])=>q.targetId===p.id).map(([id,q])=>({id,fromId:q.fromId,fromName:by(r,q.fromId)?.name||'플레이어'})),notes:z.notes||[]}); }
...
function zombieSetup(r){
  ...
  const zc=ps.length>=8?2:1,zombies=new Set(shuffle(ps).slice(0,zc).map(p=>p.id));r.round=1;r.zombie={round:1,players:{},pending:{},lastReport:null,message:'역할을 확인하십시오. 잠시 후 1라운드가 시작됩니다.',result:null};
  for(const p of ps)r.zombie.players[p.id]={status:zombies.has(p.id)?'zombie':'human',originalZombie:zombies.has(p.id),points:0,cure:1,touched:new Set(),infectedAt:null};
  r.phase='zombieRole';r.deadline=Date.now()+12000;setTimer(r,12,()=>zombieStartRound(r,1));emitRoom(r)
}
function zombieStartRound(r,n){clearTimer(r);r.round=n;r.zombie.round=n;for(const z of Object.values(r.zombie.players))z.touched=new Set();r.phase='zombiePlay';r.zombie.message=n===3?'최종 라운드입니다. 끝까지 살아남고 최대한 많은 승점을 획득하십시오.':`${n}라운드입니다. 대화하고 믿을 사람과 접촉하십시오.`;setTimer(r,600,()=>zombieEndRound(r));emitRoom(r)}
...
function zombieFinish(r){const c=zombieCounts(r);const rows=active(r).map(p=>{const z=r.zombie.players[p.id];return {id:p.id,name:p.name,status:z.status,originalZombie:z.originalZombie,points:z.points||0}});const humans=rows.filter(x=>x.status==='human').sort((a,b)=>b.points-a.points);const zombies=rows.filter(x=>x.status==='zombie').sort((a,b)=>(b.originalZombie-a.originalZombie)||(b.points-a.points));const ranking=humans.length?[...humans,...zombies]:[...zombies,...humans];r.zombie.result={humans:c.humans,zombies:c.zombies,ranking};r.phase='zombieResult';r.zombie.message=humans.length?'인간 생존자가 남았습니다. 최종 순위를 공개합니다.':'모든 인간이 감염되었습니다. 좀비의 승리입니다.';awardGeniusRanking(r,ranking.map((x,i)=>({...x,value:Math.max(0,1000-i)})));saveAccounts();emitRoom(r)}
function zombieTouchRequest(r,pid,targetId){if(r.phase!=='zombiePlay')throw Error('지금은 접촉할 수 없습니다.');if(pid===targetId)throw Error('자기 자신과 접촉할 수 없습니다.');const b=by(r,targetId),za=r.zombie.players[pid],zb=r.zombie.players[targetId];if(!b?.connected||!za||!zb)throw Error('접촉할 수 없는 플레이어입니다.');if(za.touched.has(targetId)||zb.touched.has(pid))throw Error('이번 라운드에는 이미 이 플레이어와 접촉했습니다.');if(Object.values(r.zombie.pending||{}).some(q=>(q.fromId===pid&&q.targetId===targetId)||(q.fromId===targetId&&q.targetId===pid)))throw Error('이미 접촉 요청이 진행 중입니다.');const id=uid();r.zombie.pending[id]={fromId:pid,targetId,round:r.zombie.round};emitRoom(r);return id}
function zombieTouchRespond(r,pid,requestId,accept){const q=r.zombie.pending?.[requestId];if(!q||q.targetId!==pid)throw Error('유효한 접촉 요청이 아닙니다.');delete r.zombie.pending[requestId];if(!accept){emitRoom(r);return false}if(r.phase!=='zombiePlay'||q.round!==r.zombie.round)throw Error('접촉 요청 시간이 지났습니다.');const za=r.zombie.players[q.fromId],zb=r.zombie.players[q.targetId];if(za.touched.has(q.targetId)||zb.touched.has(q.fromId))throw Error('이번 라운드에는 이미 접촉했습니다.');za.touched.add(q.targetId);zb.touched.add(q.fromId);if(za.status==='human'&&zb.status==='human'){za.points++;zb.points++}else if(za.status!==zb.status){const human=za.status==='human'?za:zb;human.status='zombie';human.infectedAt=Date.now()}emitRoom(r);return true}
function zombieCure(r,pid){if(r.phase!=='zombiePlay')throw Error('라운드 진행 중에만 치료제를 사용할 수 있습니다.');const z=r.zombie.players[pid];if(!z||z.status!=='zombie')throw Error('현재 감염 상태가 아닙니다.');if(z.originalZombie)throw Error('최초 좀비는 치료할 수 없습니다.');if(!z.cure)throw Error('치료제를 이미 사용했습니다.');if(!z.infectedAt||Date.now()-z.infectedAt>10*60*1000)throw Error('치료 가능 시간이 지났습니다.');z.cure--;z.status='human';z.infectedAt=null;emitRoom(r)}
...
io.on('connection', s => {
  ...
  s.on('zombieCure',(_,cb)=>{try{const r=rooms.get(s.data.roomCode);if(!r||r.game!=='zombie')throw Error('좀비게임 방이 아닙니다.');zombieCure(r,s.data.playerId);cb?.({ok:true})}catch(e){cb?.({ok:false,error:e.message})}});
