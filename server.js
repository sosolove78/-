...
const LIMITS = { liar:[3,10], mafia:[5,12], truth:[2,10], rummi:[2,4], wordchain:[2,10], genius:[4,10], stockwar:[4,10], deathmatch:[2,2], indianpoker:[2,2] };
const GAME_NAMES = { liar:'라이어게임', mafia:'마피아게임', truth:'진실게임', rummi:'루미큐브', wordchain:'끝말잇기', genius:'지니어스게임', stockwar:'주식전쟁', deathmatch:'데스매치', indianpoker:'인디언 포커' };
...
io.on('connection', s => {
  ...
  s.on('endGame',(_,cb)=>{try{const r=rooms.get(s.data.roomCode);if(!r||r.hostId!==s.data.playerId)throw Error('방장만 게임을 종료할 수 있습니다.');clearTimer(r);clearTimeout(r.botTimer);if(r.parentGame==='genius'||['stockwar'].includes(r.game)){r.game='genius';r.parentGame=null;r.stock=null;}else if(r.parentGame==='deathmatch'||r.game==='indianpoker'){r.game='deathmatch';r.parentGame=null;r.indian=null;}r.phase='lobby';r.round=0;r.geniusGift=null;r.geniusRewardsApplied=false;r.players.forEach(p=>{p.alive=true;p.role=null;p.rolePublic=null;p.claimedRole=null});emitRoom(r);cb?.({ok:true})}catch(e){cb?.({ok:false,error:e.message})}});
  ...
  s.on('backToLobby',(_,cb)=>{const r=rooms.get(s.data.roomCode);if(r&&r.hostId===s.data.playerId){clearTimer(r);if(r.parentGame==='genius'){r.game='genius';r.parentGame=null;r.stock=null;}else if(r.parentGame==='deathmatch'){r.game='deathmatch';r.parentGame=null;r.indian=null;}r.phase='lobby';r.players.forEach(p=>{p.alive=true;p.role=null;p.rolePublic=null;p.claimedRole=null});r.loverIds=[];r.mafiaChats={team:[],dead:[]};emitRoom(r);cb?.({ok:true});}});
